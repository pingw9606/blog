...
![图片展示的是一个“写跟进”弹窗界面。弹窗标题为“写跟进”，上方显示“test99”及“最后跟进时间：2022-10-19 11:42:08”。弹窗内有“沟通事宜”“详情及结果”“联系方式”“本次联系时间”等输入框，其中“详情及结果”输入框内显示代码“<img src='111' onerror='\\alert(111)'/>”。下方有“点击上传”按钮，右下角有“取消”和“确认”按钮。该图片与文档中介绍XSS攻击相关，展示了可能触发XSS攻击的代码示例。](./imgs/WnT2bNrdvobZJ3xH4DWcrP45nId.png)

![图片展示的是一个写跟进的界面。其中“详情及结果”输入框内有一段代码`<img src="\\'111\\'" onerror="console.log(window.localStorage)" />`，代码部分以红色高亮显示。结合上下文可知，这是在说明前端安全中XSS相关内容，此代码可能是用于演示存储型攻击的示例代码，使用者浏览页面时可能会先后触发`alert`弹框和获取`localStorage`本地数据，是典型存储型攻击场景中的一部分内容。](./imgs/ObGyb6mDeofJCRxfa3McGLUJn6d.png)

使用者浏览页面，分别先后触发了`alert`弹框和`localStorage`获取本地数据：

![图片展示了使用者浏览页面时触发的`alert`弹框和`localStorage`获取本地数据情况。页面显示“test99”及“联系日志”“操作日志”等内容，下方有“商务（电话）”“商务（QQ）”的沟通记录。右上角弹出“localhost:8080 显示”窗口，内有红色框突出显示数字“111”。这与文档中介绍的存储型XSS攻击相关，展示了攻击后触发的弹框和获取的本地数据情况。](./imgs/MPstbPeeYo0c0KxwaoUcTOaUn0g.png)

![图片展示的是浏览器开发者工具Console标签页中的内容。上方有“Elements”“Console”“Sources”等标签，当前选中“Console”。下方显示了“\[HMR\] Waiting for update signal from WDS...”等信息，以及一个以“Storage”开头的JSON数据，其中包含“userInfo”“empObj”“UserLogin”等字段，部分字段内容被红框突出显示。该图片与上下文介绍的存储型XSS攻击相关，展示了攻击后在浏览器控制台中可能看到的存储在本地的数据情况。](./imgs/ApKEbsKAIo4ImPxVcNbcT5Lqnbe.png)
...
![图片展示了基于DOM的XSS攻击示例。在页面输入框中输入`''onclick=alert(/xss/)`，点击“提交内容”按钮后，页面中`<div id="div"></div>`标签包含`<a href onclick="alert(/xss/)" >链接地址</a>`。图片中红色框标注了输入框和“链接地址”文本，还用红色框标注了页面中生成的包含XSS代码的`<a>`标签。该图片直观呈现了基于DOM的XSS攻击原理，与上下文介绍的XSS攻击相关知识相呼应。](./imgs/JwadbgvA0oNWAxxrfLBc99VQnff.png)
...
![图片展示了一个数据库图标，上方有“SQL”字样，右侧有一个注射器，注射器上写着“注入”二字。该图片位于介绍SQL注入攻击的上下文部分，直观地呈现了SQL注入攻击的概念，即通过将恶意SQL查询或添加语句插入到应用的输入参数中，在后台SQL服务器上解析执行进行攻击，与上下文对SQL注入攻击的描述相呼应，帮助理解这一攻击方式。](./imgs/D1B9bActXooSLXxmFf9cmMI6n7c.png)
...
![图片展示了前端鉴权流程。左侧为浏览器端，用户发起请求，返回未登陆错误码，重定向至login页面，输入用户名密码登陆，浏览器将cookie保存在本地，每次请求都会带上cookie。右侧为服务器端，服务端验证request中是否有sk，返回失败或结果。该图与文档中前端鉴权流程的上下文对应，直观呈现了请求与响应的交互过程。](./imgs/GJjMbV8wcoNZiNx3zk4cIFZKnXb.png)
...
![图片是一张关于Cookie属性的表格，展示了不同属性的KEY与VALUE。其中，Name对应名称，Value对应值，Domain为可访问此cookie的域名，Path是可访问的页面路径，Size表示大小。HttpOnly若为true则不能通过document.cookie访问；Secure为true时只能通过https传递；Expires/Max - Age用于设置清除cookie的时间；SameSite可限制第三方Cookie。该图片与文档中关于Cookie属性的内容相呼应，直观呈现了相关属性信息。](./imgs/Jup9b9amAokn7QxqzUAcOBa4nOf.png)
...
![图片展示了前端鉴权流程。左侧为浏览器端，用户发起请求，返回未登陆错误码，重定向至login页面，输入用户名密码登陆，请求头中带上token；右侧为服务器端，服务端验证request中是否有token，没有返回失败，验证用户名密码，生成token返回，若有token则验证其有效性，返回结果。该图直观呈现了前端鉴权的请求与响应过程，与文档中前端鉴权流程的描述相契合。](./imgs/Hs9mbunDvo1KPfxjzUNc0dAfnpg.png)
...
![图片展示了单点登录（SSO）流程。客户端访问系统A时，重定向到SSO，因无SSO凭证需登录，输入账号密码后获取ticket（token），存入本地，访问系统A时凭ticket验证凭证。若凭证有效，处理业务；若无效，需登录。访问系统B时，重定向到SSO，因有ticket，访问系统B时凭ticket验证凭证，凭证有效处理业务，凭证无效需登录。该图与文档中介绍的单点登录流程内容对应，直观呈现了其操作步骤。](./imgs/RkWfblZqBoJ4fOx9e6DcqKsan9d.png)

对浏览器来说，SSO 域下返回的数据要怎么存，才能在访问 A 的时候带上？这就需要也只能由 A 提供 A 域下存储凭证的能力。

![图片展示了基于SSO的登录流程。浏览器访问系统A，若无SSO cookie，需登录；登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统A，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie，需登录，登录成功后，重定向至SSO，若无cookie，需登录，登录成功后，重定向至系统B，若无cookie](./imgs/Ni4lbg6gBoB4mwxEI5Vc3l88nDd.png)
...
![图片展示了OAuth2协议的流程。Client（客户端）向Resource Owner（资源所有者）发送Authorization Request（授权请求，A），获得Authorization Grant（授权许可，B）。接着Client向Authorization Server（授权服务器）发送Authorization Grant（C）以请求Access Token（访问令牌），若验证有效，服务器颁发令牌（D）。Client再凭借Access Token向Resource Server（资源服务器）请求Protected Resource（受保护资源，E），Resource Server验证令牌有效后响应请求（F）。此图直观呈现了OAuth2为第三方应用颁发令牌获取资源的流程。](./imgs/MuvLb5zgkokcLcxsDa2cqiTmn2d.png)
...
![图片展示了2FA（双因素认证）中生成TOTP（时间同步一次性密码）的计算公式。。公式分为两部分，TC = Math.floor(Date.now() / 1000) / 30，用于计算时间戳；TOTP = HASH(SecretKey, TC)，通过SecretKey和TC计算出6位随机数。该图片与文档中介绍2FA认证方式的内容相关，是对线上的boss必须开启二次认证，生成二维码并计算6位随机数这一过程的数学公式说明。](./imgs/FZ5qbsZ7UomjOcx8U4hcXeTknSc.png)
...
![图片是一张表格，列出了WebSocket握手阶段需要用到的HTTP头及其相关说明。其中“Host”“Upgrade”“Connection”“Sec - WebSocket - Key”“Sec - WebSocket - Version”“Sec - WebSocket - Accept”等HTTP头均为必须项，且有具体解释；“Origin”“Sec - WebSocket - Protocol”“Sec - WebSocket - Extensions”为可选项，有相应说明。该表格与上文介绍的WebSocket握手阶段需要用到的HTTP头相呼应，为理解WebSocket握手过程提供了关键信息。](./imgs/PTg4buahdoSMZNxgSKdchoebnOd.png)

一次完整的握手连接如下图:

![图片展示了WebSocket协议握手阶段的HTTP请求和响应内容。左侧为HTTP请求，包含GET、?encoding=text、HTTP/1.1等信息，User-Agent为Mozilla/5.0等，Origin为https://www.websocket.org等。右侧为HTTP/1.1 101 Web Socket Protocol Handshake响应，有Access-Control-Allow-Origin等头信息，Connection为Upgrade，Date为2017年8月13日11:07:18 GMT，Server为Kaazing Gateway，Upgrade为websocket。该图片与上文介绍的WebSocket握手阶段需要用到的HTTP头相呼应，直观呈现了握手过程中的HTTP交互内容。](./imgs/LxX9bJ2vtopOoqxLMI4cC1Ipnag.jpg)
...
![图片展示了跨站点WebSocket劫持攻击的示意图。受害者的Web浏览器向合法的WebSocket启用的web服务器（https://ws - example - site.com）发送恶意WebSocket连接打开请求，请求中包含Origin头为evil - website.com、token为125838121231的内容。同时，攻击者操作的web服务器（https://evil - website.com）也向该web服务器发送恶意WebSocket连接打开请求，但请求中Origin头为evil - website.com，且无认证token。此图直观呈现了跨站点WebSocket劫持攻击的流程，与上文提到的该漏洞危害大但易被忽视的内容相呼应。](./imgs/ZuSmb11oJoyiSuxWbfRcX72gn4b.jpg)
...
![图片展示了WebSocket攻击示例。左侧服务器端发出“1 Give me the page!”，中间客户端收到后执行“2 <script src=“chat.js”>”，接着客户端尝试建立WebSocket连接“2.1 new WebSocket(“ws://darksite”);...”，但被标记为“FAIL!”，最后服务器端显示“3 Cool, I am chatting with CS!”。该图与上下文关系紧密，直观呈现了WebSocket应用中可能遭遇的跨站脚本攻击，强调了对输入进行校验的重要性。](./imgs/H8dUbhlMkozD75xbSLnc5UpHnKh.jpg)
...
![图片是一张表格，展示了不同请求类型在正常情况和Lax设置下是否发送Cookie。请求类型包括链接、预加载、GET、POST、iframe、AJAX、Image等，每种类型有示例代码，正常情况和Lax设置下是否发送Cookie也分别给出。如链接和预加载在正常情况和Lax下均发送Cookie，GET表单在正常情况发送Cookie，Lax下不发送等。该表与上下文介绍的导航到目标网址的GET请求情况相呼应，帮助理解不同请求类型在不同设置下的Cookie发送情况。](./imgs/VgP9broHdoFNZcxYHndcUxHinKc.png)
