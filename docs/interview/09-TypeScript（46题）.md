...
![图片展示了TypeScript中重复声明变量“a”的错误提示。代码中在同一个文件中声明了两次const变量a，编译时弹出“无法重新声明块范围变量“a””的错误信息，指出1.ts(3, 7)处也声明了“a”，并提示有可用的快速修复。该图片与上文提到的TypeScript中变量重复声明问题相关，直观呈现了错误情况，帮助理解在TypeScript中声明变量时需注意避免重复声明。](./imgs/XHlQbGYlaoAMY9xqkKacLIq1nwg.png)
...
![图片展示了TypeScript与JavaScript、TypeScript与ES5/ES6的关系。左侧图中，TypeScript与JavaScript部分重叠，表示TypeScript是JavaScript的超集，支持ES6语法，可运行JavaScript代码。右侧图中，TypeScript包含ES6，ES6又包含ES5，说明TypeScript扩展了JavaScript语法，支持ES6，且兼容ES5。这些图直观呈现了TypeScript与JavaScript、不同版本ECMAScript的关系，与文档中对TypeScript是JavaScript超集、支持ES6语法等的解释相契合。](./imgs/BiP5bOnKFo1thPxti9qcpTeKnwd.png)
...
![图片是一张对比表，将JavaScript和TypeScript在语言、学习难度、类型、客户端/服务端、拓展名、耗时、数据绑定、语法、静态类型、模块支持、接口、可选参数方法、原型等13个方面进行对比。如JavaScript是脚本语言，TypeScript是面向对象编程语言；JavaScript学习难度灵活易学，TypeScript需要有脚本编程经验；JavaScript类型是轻量级解释编程语言，TypeScript是强类型的面向对象编程语言等。该表直观呈现了两者在各方面的差异，与上下文介绍TypeScript是JavaScript的超集，扩展了JavaScript的语法等内容相呼应。](./imgs/T7y2bA4OMoZf4OxwMV4c3i4Lnmd.png)
...
![图片展示了一个TypeScript代码示例，定义了一个泛型类`Stack`。类中包含`arr`私有属性，类型为泛型参数`T`。`push`方法接收泛型参数`item`，将其添加到`arr`中。`pop`方法从`arr`中移除最后一个元素。最后创建了一个`Stack`实例，传入`boolean`类型，编译时提示`boolean`不满足约束`Params`。该图片与上下文紧密相关，通过具体代码示例，直观呈现了TypeScript中泛型的使用及约束类型的应用。](./imgs/GpsbbFZPXoACpkxOnmrc4Czvn3d.png)
...
![图片展示了一个TypeScript函数示例。函数名为`getValue`，接受两个泛型参数`T`和`U`，其中`T`约束为`object`，`U`约束为`keyof T`。函数接收一个对象`obj`和一个属性`key`，返回`obj\[key\]`。示例中定义了对象`a`，包含`name`和`age`属性，类型分别为`string`和`number`。在`getValue(a)`处，代码提示`key: "name" | "age"`，返回类型为`string | number`，表明函数能根据传入对象属性类型返回相应值。](./imgs/TeGebySZUoZ1XDxMri1cRDuyn5b.png)
