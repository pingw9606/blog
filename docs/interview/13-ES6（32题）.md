...
1. 导出的是值的拷贝： 但对于对象和数组等引用类型，修改引用类型的属性会在所有引用中反映出来。
...
![图片展示的是Map数据结构的使用示例代码。首先创建一个Map对象，初始化时传入键值对数组，包含键为'num'、值为99的键值对。接着使用set方法将键'num'对应的值改为99。最后通过console.log输出Map对象，显示键值对为{'user' => '小如', 'num' => 99}。该图片与上文介绍Map数据结构的内容相呼应，直观呈现了Map的创建、设置键值及输出结果。](./imgs/ElEvbsxqUoyCqkxqquscVh6gnNc.png)
...
![图片展示了Map数据结构的打印结果。在浏览器控制台中，Map被定义为一个键值对结构，其中键为对象，值为“111”。键的对象包含“name”和“age”属性，分别对应“小如”和9。该图片与文档中介绍Map数据结构的内容相关，直观呈现了Map键值对的存储形式，帮助理解Map数据结构的键值对属性及数据存储方式。](./imgs/VjhqbWPjuoCQDjxwxGRcODaAnWf.png)
...
![图片展示了Map数据结构的打印结果。在浏览器控制台中，Map被展开显示为一个键值对结构，键为“Amy”，值为“男”。其下还有“Entries”数组，包含一个键值对，键为0，值为{“Amy” => “男”}。此外，还有“size”属性显示大小为1，“\[\[Prototype\]\]”属性指向Map。该图片与文档中介绍Map数据结构的内容相关，直观呈现了Map的结构形式。](./imgs/X2Rob0J14old5wxqeuqcdLDVnDg.png)
...
![图片展示了在浏览器控制台中创建Set对象并打印其内容的操作及结果。代码为“var set = new Set(\[1,2,3,5,6\]) console.log(set)”，执行后控制台输出“Set(5) {1, 2, 3, 5, 6}”。这与文档中介绍Set对象类似于数组且成员值唯一的上下文对应，直观呈现了Set对象的打印结果，帮助理解Set数据结构的特点。](./imgs/E832bqNgMob0MHxNzakchFvJnhC.png)
...
![图片展示了ES6中class类的使用示例。代码定义了一个名为Foo的类，包含构造函数和一个like方法。构造函数接收color参数并赋值给this.color，like方法在控制台打印`like${this.color}`。执行Foo()时，报错“Class constructor Foo cannot be invoked without 'new'”，表明class类执行时需使用new关键字。此图与文档中介绍class类执行需new关键字的内容相呼应，直观呈现了这一特性。](./imgs/GOggbvWy5oQqRvxS8zpc5ZobnwD.png)

class类执行的话会报错，而es5中的类和普通函数并没有本质区别，执行肯定是ok的。

二、class类不存在变量提升

![图片展示了一段JavaScript代码示例，用于说明class类无法遍历实例原型链上的属性和方法。代码中定义了一个名为Foo的函数式构造函数，其构造函数内部将color属性赋值给this。接着定义了Foo.prototype.like方法，用于打印当前实例的color值。之后定义了一个普通函数f，其内部也尝试打印this.color。最后创建了一个Foo实例foo，但执行foo.like()时，由于class类无法遍历实例原型链，导致console.log语句无法打印出color值，仅输出了undefined。](./imgs/Iaetbjd6MotYhnxSMx5cBSfJnVf.png)

![图片展示了一段JavaScript代码及运行结果。代码中定义了class类Foo，包含构造函数constructor和方法like，之后尝试使用new关键字实例化Foo类。运行结果出现红色报错信息“Uncaught ReferenceError: Foo is not defined”，表明类未定义。这与上下文提到的“class类不存在变量提升”相呼应，通过此代码示例直观地说明了class方式没有把类的定义提升到顶部，从而导致实例化时类未定义的报错情况。](./imgs/Nw7jbiLtIoQ5q1xrm6ccyEpmnme.png)
...
![图片展示了ES6中Promise的流程。初始状态为pending，当异步操作成功时，通过fulfill进入.then(onFulfillment)，返回新的pending状态Promise；若异步操作失败，经reject进入.then(onRejection)或.catch(onRejection)进行错误处理，同样返回新的pending状态Promise。此外，还有settled指向async actions，表明相关异步操作。该图与上文对Promise状态、特点的介绍相呼应，能帮助理解Promise从一种状态到另一种状态的流转及相关操作。](./imgs/TAGZbul2AopLAoxD5ICcLudYnhf.png)
...
<sheet sheet-id="2cgEAz" token="ZHI2sxPI8hxNGctdNVLcGteEn9g"></sheet>
