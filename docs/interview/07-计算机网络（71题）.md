...
![图片展示了TCP三次握手的流程。客户端处于SYN_SENT状态，发送SYN=1，seq=J的请求建立连接包；服务器端处于SYN_RCVD状态，收到后回复SYN=1，ACK=1，ack=J+1，seq=K的确认应答包，并请求建立连接；客户端再回复ACK=1，ack=K+1的确认应答包，至此客户端和服务器进入ESTABLISHED状态。该图直观呈现了三次握手的步骤和状态变化，与上文对三次握手的描述相呼应。](./imgs/IaqvbhPX8oZAi9xjtxYcp1c9nDe.png)
...
![图片展示了Webkit解析渲染页面的过程。从HTML和CSS解析开始，HTML经HTML Parser生成DOM树，CSS经CSS Parser生成CSSOM树。接着，DOM树与CSSOM树结合生成渲染树，再通过Layout生成布局，Painting进行绘制，最终Display显示页面。此图与上下文紧密相关，直观呈现了文档中提到的浏览器解析渲染页面的步骤，帮助理解从HTML和CSS到页面显示的整个流程。](./imgs/Njj9b5k9SovcvbxQDAjc7G7gnMf.png)
...
![图片展示了使用wireshark抓取的https握手过程数据包。源地址为10.29.48.18，目标地址为220.181.38.149，协议为TCP。数据包序号、ack、长度等信息依次呈现，如序号54的数据包序号为66，ack为64240，长度为1460等。其中，序号57的数据包序号为54，ack为8192，长度为0，序号59的数据包序号为54，ack为132096，长度为0。该图片与上文介绍的使用wireshark抓取https握手过程数据包的内容相呼应，直观呈现了数据包的具体信息。](./imgs/VgOcbyyUIo95Dgx0ughcJNJonEg.png)

![图片展示的是使用wireshark抓包工具抓取的https握手过程数据包。其中，序号71的220.181.38.149向10.29.48.18发送了TLSv1.2的60 Change Cipher Spec数据包；序号72的10.29.48.18向220.181.38.149发送了99 Encrypted Handshake Message数据包；序号73的220.181.38.149向10.29.48.18发送了54 57555 → 443 \[ACK\] Seq=305 Ack=4388 Win=131328 Len=0的数据包。这些数据包是https握手过程中的关键交互，对应文档中对https握手过程的抓包解释。](./imgs/Bol4bfVa5oAdVXx38u7cThHnnth.png)

遇到凡事不要慌，接下来待我给你慢慢道来（ack消息属于tcp协议里面的确认报文，不做解释）

**第一步**

![图片展示了Wireshark抓包工具中https握手过程的网络数据包。序号54的数据包显示客户端向服务器（220.181.38.149:443）发送SYN报文，Seq=0、Win=64240、Len=0、MSS=1460、WS=256、SACK_PERM=1；序号56的数据包显示服务器向客户端发送SYN+ACK报文，Seq=0、Ack=1、Win=8192、Len=0、MSS=1452、WS=32、SACK_PERM=1。该图片对应文档中“第一步”内容，直观呈现了tcp三次握手的网络数据包情况。](./imgs/PFPdbZdBboTGUJxmCkucs1BEnWg.png)

解释说明：tcp三次握手，这个不做解释，如果这块不清楚，比如ack，seq,mss,win都代表什么意思，这个可以在互动区留言，我视情况专门写几篇tcp的文章（这块太大了，没几篇是介绍不完的）

**第二步：客户端发送client_hello**

![图片展示了Wireshark抓包工具中https握手过程的第二步客户端发送的“client_hello”数据包内容。其中，红色框突出显示了版本为TLS 1.2、客户端随机串、客户端支持加密套件列表、客户端支持压缩算法列表等关键信息。该图片与上文提到的用curl命令抓取https建立链接成功前的过程相呼应，直观呈现了客户端发送“client_hello”时所包含的具体数据内容，帮助理解https握手过程中的客户端行为。](./imgs/DV37bWyZJocmiwx2UiucDFyZngh.png)
...
![图片展示的是Wireshark抓包分析中TLSv1.2的server_hello报文信息。报文长度为48字节，包含TLS协议版本、随机数、Cipher Suite、Compression Method等字段。其中，Cipher Suite为TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256，Compression Method为null。该图片与上文介绍的https握手过程中的服务端发送server_hello步骤对应，直观呈现了server_hello报文的具体内容，帮助理解服务端发送协商信息的结果。](./imgs/HKjtb0QNPoCeKixaGT0ch75Cnzf.png)
...
![图片展示了HTTPS握手过程中服务端发送证书的TCP报文信息。报文源地址为220.181.38.149，目标地址为10.29.48.18，协议为TCP，端口分别为443、57555、57555。报文长度分别为841、1660、1660，共分3个报文发送，总长度3761字节。其中，报文61、62、63的payload分别为0 - 1459、1660 - 2919、2920 - 3760字节。该图片与上文解释服务端发送证书时因报文长度较长需分段发送的内容相呼应，直观呈现了分段情况。](./imgs/W4aPbsRZQoo27nxcfcpcs4YinBf.png)
...
![图片展示了SSL握手过程中的Server Key Exchange报文内容。报文类型为Server Key Exchange，长度329字节，使用EC Diffie-Hellman算法。关键信息包括Curve Type为named_curve，Named Curve为secp256r1，Pubkey Length为65，Pubkey为一段65字节的十六进制数，Signature Algorithm为rsa_pkscs1_sha256，Signature Length为256，Signature为一段256字节的十六进制数。该图片与上下文介绍的第五步服务端发送Server Key Exchange内容相契合，直观呈现了报文中的具体参数。](./imgs/ZjHZbwJJdoATshxAC2GcYvT3nSh.png)
...
![图片展示的是SSL握手过程中的Server Hello Done报文。报文长度为63字节，协议版本为TLS 1.2，长度为4字节，Handshake Type为Server Hello Done，长度为0。该报文是服务端发送的，用于通知客户端server_hello信息发送结束，对应文档中“第五步：服务端发送Server Key Exchange”之后的“第六步：服务端发送Server Hello Done”步骤，是SSL握手流程中的关键报文。](./imgs/H1Ptb5zI0o86lCx5Gpqc2nM7nhf.png)

解释说明:通知客户端 server_hello 信息发送结束

**第七步：客户端发送.client_key_exchange+change_cipher_spec+encrypted_handshake_message**

![图片展示了SSL/TLS握手过程中客户端发送的报文内容。报文包含Client Key Exchange、Change Cipher Spec、Encrypted Handshake Message三个部分。Client Key Exchange部分，Handshake Protocol为Client Key Exchange，Type为Client Key Exchange，长度为66字节；Change Cipher Spec部分，Content Type为Change Cipher Spec，Version为TLS 1.2，Length为1字节；Encrypted Handshake Message部分，Content Type为Handshake，Version为TLS 1.2，Length为40字节。该图片与上下文介绍的SSL握手过程中的客户端发送报文内容相契合。](./imgs/ZeQUblUOIoWwFNx7DBQcjLUZnic.png)
...
![图片展示了网络抓包工具捕获的HTTPS握手过程中服务端发送New Session Ticket的数据包信息。源IP为70.2.121.139，目的IP为220.181.38.149，协议为TLSv1.2 。数据包内容显示了记录层的握手协议为New Session Ticket，包含版本、长度、握手类型等信息，还具体列出了TLS会话票据的生命周期最小值、长度及票据内容等。这与上下文提到的服务端发送New Session Ticket步骤相呼应，直观呈现了该步骤的数据包细节。](./imgs/Yrmebvlw0of3f5x1KoBcCATmnJg.png)

解释说明:服务器给客户端一个会话，用处就是在一段时间之内（超时时间到来之前），双方都以协商的密钥进行通信。

**第九步：服务端发送change_cipher_spec**

![图片展示的是计算机网络中https握手过程中的第九步，服务端发送change_cipher_spec报文的Wireshark抓包分析。报文内容显示为60字节，版本为TLSv1.2，长度为1，Content Type为Change Cipher Spec（20），Version为TLS 1.2（0x0303），Length为1，Change Cipher Spec Message为“”。该图片与上文对服务端发送change_cipher_spec报文的解释说明相呼应，直观呈现了报文的具体信息。](./imgs/Ck9hbOztdoSBKCx72pTc1NTIn1f.png)
...
![图片展示的是计算机网络中https握手过程中的第十步：服务端发送encrypted_handshake_message。画面中呈现了网络抓包信息，包括帧编号、字节长度、捕获字节长度、接口、以太网帧、源MAC地址、目标MAC地址、IP协议、源IP地址、目标IP地址、TCP协议、源端口、目标端口、序列号、确认号、长度等。关键部分是Transport Layer Security下的TLSv1.2 Record Layer信息，显示Content Type为Handshake（22），Version为TLS 1.2（0x0303），Length为40，Handshake Protocol为Encrypted Handshake Message，与上下文解释说明相呼应。](./imgs/BGyBbeaX4oFCrcxDt0ncC1sLnTf.png)

解释说明:目的同样是测试密钥的有效性，客户端发送该报文是为了验证服务端能正常解密，客户端能正常加密，相反：服务端发送该报文是为了验证客户端能正常解密，服务端能正常加密

**第十一步：完成密钥协商，开始发送数据**

![图片展示的是Wireshark抓包分析界面，重点突出第78和79帧的Reassembled TCP Segments。第78帧的payload为v-1459，第79帧的payload为1460 - 2871。该图片与文档中计算机网络https握手过程的第十一步“完成密钥协商，开始发送数据”相关，用于直观呈现数据分段发送的情况，辅助理解数据在TCP层的传输状态。](./imgs/LKsBbntTEorMItxCF0KcZwmhn2d.png)

解释说明：数据同样是分段发送的

**第十二步：完成数据发送，4次tcp挥手**

![图片展示了Wireshark抓包分析中SSL/TLS协议的报文信息。红框突出显示了88号报文，其源IP为220.181.38.149，目标IP为10.29.48.18，协议为TCP，端口为54，报文内容为TLSv1.2 Encrypted Alert。该报文对应文档中第十二步“完成数据发送，4次tcp挥手”中客户端或服务器发送的“加密通信因为某些原因需要中断，警告对方不要再发送敏感的数据”这一内容，是加密通信中断的警示报文。](./imgs/QjCEbWCvzoUnvwxFl2ZcOCstnBc.png)
...
![图片是一张HTTPS握手过程的流程图，展示了客户端与服务端的交互步骤。从客户端发送hello_request开始，经server_hello、client_hello等报文交互，到server_hello_done、certificate、server_key_exchange等报文传输，再到client_key_exchange、change_cipher_spec、encrypted_handshake_message等报文交互，最后完成密钥协商，开始发送数据。图中还标注了部分报文的类型、长度等信息，以及部分报文的名称和内容，如server_hello包含server_random、server_session_id、server_certificate等字段。](./imgs/PitIbXCgHoKO7IxfFMGcMGD5nxb.png)
...
![图片展示了HTTP/1.1和HTTP/2的请求响应流程。左侧为HTTP/1.1 Baseline，显示从打开连接到客户端渲染页面的8个步骤，各步骤间有箭头表示顺序。右侧为HTTP/2 Multiplexing，同样从打开连接到客户端渲染页面，但各步骤间有交织的箭头，表明多个消息交织工作，效率更高。图片直观呈现了HTTP/2多路复用的原理，与上下文解释HTTP/2中消息交织工作，提高通信效率的内容相契合。](./imgs/EssVbiWXEogiJ9xOpyucC9lQnXd.png)
...
![图片展示了HTTP/2帧结构。前9个字节为固定部分，包含Length（24）、Type（8）、Flags（8）、Stream Identifier（31）等字段。Length字段用于表示帧数据的长度；Type字段标识帧类型；Flags字段有多个标志位，如R位；Stream Identifier字段用于标识流；Frame Payload字段为帧数据内容。该图与上下文紧密相关，直观呈现了HTTP/2帧结构，帮助理解帧的字节组成及各字段作用。](./imgs/EZcvbANTdoqcNgxIUt8c4iymnzh.png)
...
![图片展示了HTTP/2协议上POST请求数据流“复用”的过程。上方显示客户端和服务器，中间有两条消息流，分别用黄色和蓝色标识。下方有三条数据流，每条数据流由多个DATA和HEADERS帧组成，DATA帧为紫色，HEADERS帧为黄色。其中，消息流1的数据流中DATA帧和HEADERS帧交错出现，消息流2的数据流中DATA帧和HEADERS帧也交错出现。该图与上下文紧密相关，直观呈现了HTTP/2多路复用的帧序列交换原理。](./imgs/NNVUbgv2HoRGXixP8DEcPCXjnvc.png)
...
![图片展示了TCP三次握手过程。客户端处于CLOSE状态，服务端处于LISTEN状态。客户端随机初始化序号client_isn，将SYN标志置1，发送SYN报文。服务端收到后，将SYN标志置1，回复SYN+ACK报文，ACK数为client_isn+1，序号为server_isn。客户端收到后，将ACK数置为server_isn+1，进入ESTABLISHED状态，服务端也进入ESTABLISHED状态，至此三次握手完成，可进行数据传输。](./imgs/LikFbDjpCobvdRxHb6vcOUaDnmb.png)
...
![图片展示了TCP协议中三次握手和四次挥手的两种情况。左侧“正常情况”下，客户端和服务器端通过三次握手建立连接，客户端发送SYN请求，服务器端回复SYN+ACK，客户端再回复ACK。右侧“客户端未收到报文”情况下，客户端发送的SYN请求丢失，服务器端回复的SYN+ACK也被丢弃，客户端超时重传，服务器端因无三次握手而认为端口占用，需重新开启端口连接。此图直观呈现了三次握手和四次挥手的差异及可能的连接问题。](./imgs/KKGhbBmP9oCcnOxcbRuc3oOpnvb.png)

还有一种情况是已经失效的客户端发出的请求信息，由于某种原因传输到了服务器端，服务器端以为是客户端发出的有效请求，接收后产生错误。

![图片展示了TCP连接建立的两种情况。左侧为正常情况，客户端（蓝色）和服务器端（绿色）通过三次握手建立连接，客户端发送SYN请求，服务器端回复SYN+ACK，客户端再回复ACK。右侧为报文延迟导致失效情况，客户端发起新的请求，因报文延迟，失效请求先到，服务器端响应了失效请求，导致连接建立失败。该图与上下文解释了“第三次握手”确认连接有效性的必要性，直观呈现了失效请求导致的连接问题。](./imgs/Z0UHbYwoBozwgTxq1d1cfZIpnPe.png)
...
![图片展示了TCP断开连接时的四次挥手过程。左侧为客户端，右侧为服务器端，双方初始状态均为ESTABLISHED。客户端主动关闭连接，发送FIN报文进入FIN_WAIT_1状态；服务器端收到FIN后回复ACK，进入CLOSED_WAIT状态，客户端收到ACK进入FIN_WAIT_2状态；之后服务器端发送FIN，进入LAST_ACK状态，客户端收到FIN后回复ACK，等待2MSL时间后进入CLOSE状态，服务器端收到ACK也进入CLOSE状态。此图直观呈现了四次挥手各阶段状态及报文交互，与上下文对四次挥手的文字说明相呼应。](./imgs/Ofc5bXwJGoOo2VxrIHuccPo9ndu.png)
...
![图片展示的是巴西Bradesco银行的网站界面。左侧有银行标志及导航栏，可选择账户、产品、服务等。中间部分以紫色背景为主，上方有“CÍRIO DE NAZARÉ”字样，下方文字介绍了解更多信息。画面右侧有两位女性和一位男性，背景是教堂等建筑。右侧还有“Assista!”按钮。该图片与文档中介绍DNS劫持现象的内容相关，可能用于说明网站被黑、被歹意镜像等危害。](./imgs/V1CQbHpljoAEvMxApUlc0nyanYb.png)
...
![图片展示了DNS劫持前后的情况。左侧为正常交易，用户询问“bank的IP是多少？”给正常DNS服务器，其回复1.2.3.4，用户再向bank发送请求，完成正常交易。右侧为被劫持，用户同样询问“bank的IP是多少？”给钓鱼DNS服务器，其回复5.6.7.8，用户向5.6.7.8发送请求，导致被骗。此图直观呈现了DNS劫持中用户、DNS服务器及目标网站间的信息交互过程，与上下文解释DNS劫持中第三方服务器参与获取目标网站IP的原理相契合。](./imgs/WNbnbAqEDoZz9dxKeufcf3dBnkc.png)
...
![图片展示了TLS 1.3握手过程中的，以浏览器和服务器为例，说明了其握手流程。浏览器先发送`client_params`、`client_random`、TLS版本、加密套件列表等信息，服务器验证数字证书后，发送`server_random`、`server_params`、确认TLS版本、使用加密套件列表、服务器使用的证书等信息。接着，浏览器和服务器分别计算`pre_random`，再计算出`secret`，之后传输使用`secret`加密。该图与上下文介绍的TLS 1.3握手流程内容相契合，直观呈现了流程步骤。](./imgs/MTyMbqkXOofExBxlkl4cavAdnpe.png)
...
![图片展示了TLS 1.2握手协议过程。浏览器发送client_random、TLS版本、加密套件列表给服务器，服务器返回server_random、server_params、确认TLS版本、使用的加密套件列表、服务器使用的证书。浏览器验证数字证书通过后，生成client_params，ECDHE计算pre_random，计算出secret，之后的传输都使用secret作为密钥进行加解密。该图与上下文紧密相关，直观呈现了TLS 1.2握手流程，帮助理解其工作原理。](./imgs/TRwbbLP2FoBNdGx5oqQco8ATnXc.png)
...
![图片展示了HTTP请求报文的结构。上方是请求行，包含Method（方法）、SP（空格）、URI（统一资源标识符）、SP、Version（版本）等字段，Method为黄色，URI为绿色，Version为蓝色。下方是请求头，有多个Field Name（字段名）和Field Value（字段值）对，字段名与值间有冒号分隔，且字段名不允许出现空格和下划线。最下方是空行，有CRLF标识。该图与上下文介绍的HTTP报文结构相关，直观呈现了请求行和请求头在报文中的位置及格式。](./imgs/GLB8bbY0WoU7vrxF33DcXg6CnTn.png)

![图片展示了HTTP响应头在报文中的结构。最上方是状态行，包含Version（版本）、Status Code（状态码）、Reason（原因）等部分。下方是响应头区域，由多个Field Name（字段名）和对应的Field Value（字段值）组成，字段名后紧接着冒号。底部是用于区分头部和实体的空行，标注为CRLF。该图与上下文紧密相关，上下文提及要展示请求头和响应头在报文中的位置，此图直观呈现了响应头相关结构，辅助理解HTTP报文结构中响应头部分的构成。](./imgs/Dv15bwwE1oBboOxZI5kcrwdvnEb.png)
...
![图片展示了使用对称密钥进行加密和解密的过程。张大胖的消息通过加密算法，在密钥作用下变成密文，经网络传输给Bill。Bill再使用解密算法和相同的密钥将密文还原为张大胖的消息。图中以箭头清晰呈现了消息、加密解密算法、密文及密钥之间的关系。该图与上下文紧密相关，上下文探讨了使用对称密钥加密的方案及存在的问题，此图直观呈现了对称密钥加密和解密的具体流程，辅助理解对称密钥加密的原理。](./imgs/GXn5blsrQoifOaxmetbcC4MLnog.png)
...
![图片展示了非对称密钥中的RSA算法原理。画面中，人物Bill分别指向红色和绿色的钥匙，红色钥匙旁标注“Bill的私钥（保存好，万万不能泄露）”，绿色钥匙旁标注“Bill的公钥（地球人都可以知道）”。该图与上下文紧密相关，用于说明RSA算法中，双方必须协商一对密钥，一个私钥一个公钥，私钥需保密，公钥可公开，体现了非对称密钥在安全性上的优势。](./imgs/Rt4ibd4UeogpKcxlGDWca4e2nMh.png)

RSA（非对称加密算法）：双方必须协商一对密钥，一个私钥一个公钥。用私钥加密的数据，只有对应的公钥才能解密，用公钥加密的数据， 只有对应的私钥才能解密。

![图片展示了非对称密钥+对称密钥结合的加密传输过程。张大胖将消息加密后发送，使用的是Bill的公钥；Bill收到密文后，使用自己的私钥解密获取消息。密文在网络传输过程中处于加密状态，确保消息安全。该图与上下文紧密相关，直观呈现了非对称密钥加密中公钥加密、私钥解密及密文传输的步骤，辅助理解HTTPS安全机制。](./imgs/A2FcbaGaAowey9xnmcIcA3Fun1b.png)
...
![图片展示了数字签名的生成过程。左侧为Bill的公钥、个人信息及其他信息，经Hash算法生成消息摘要。接着用CA的私钥对消息摘要加密，得到数字签名。该图与上下文紧密相关，是对上文“数字签名相当于公证处在公证书上盖章”的具体说明，直观呈现了数字签名的生成步骤，帮助理解数字证书中数字签名的生成原理。](./imgs/WoXjbyWTloFxtUxLtmtcwiN9n3g.png)

数字签名也就是相当于公证处在公证书上盖章。

![图片展示了 addCriterion](./imgs/YAhzbSkaIoUFtPxGScQcFLivnkf.png)

数字签名和原始信息合在一起称为数字证书，Bill 只需将数字证书发送给张大胖就可以了。

在拿到数字证书之后，就用同样的Hash 算法， 再次生成消息摘要，然后用CA的公钥对数字签名解密， 得到CA创建的消息摘要， 两者一比，就知道有没有人篡改了！

![图片展示了数字中关于数字证书的验证过程。左侧是数字证书，包含Bill的公钥、个人信息、其他信息及数字签名。中间用Hash算法对这些信息生成消息摘要。右侧分别用数字签名和CA的公钥解密生成消息摘要，最后比较两个消息摘要是否是否相同。该图直观呈现了数字证书中信息的处理及验证方式，与上文介绍的数字签名和数字证书用于验证信息是否被篡改的内容相呼应。](./imgs/K81abiGZ1odOG1x1oLOcSAVKnNf.png)
...
![图片展示了Axios的目录结构。其中/lib/为项目源码目录，其下包含多个子目录及文件。/adapters/定义发送请求的适配器，含适用于node环境的http.js和浏览器环境的xhr.js；/core/包含如axios.js（入口，创建构造函数 ）、InterceptorManager.js（拦截器管理器）等核心功能文件；还有/utils/公用工具、defaults.js默认配置等。此图与上文提到的先看Axios目录结构的内容相呼应，直观呈现其源码构成。](./imgs/SLg4bW03Wo6mHYxoJYTcjwTwnpd.png)
...
![图片展示了Axios请求流程图。从axios开始，经axios.create()生成axios实例，再通过createInstance执行/命名执行，接着调用Axios.prototype.request，进入interceptors，处理参数与默认参数/transformdata后，执行dispatchRequest，再通过adapter，最后根据报错/cancel结果，分别指向axios.rejected或axios.fulfilled。该图与上下文紧密相关，直观呈现了Axios请求的执行路径及关键环节。](./imgs/Q3L8bUPT4oXLZjxY5XfcypWYncb.png)
...
![图片展示了HTTP/2与HTTP over QUIC（HTTP/3）的协议栈结构。左侧为HTTP/2，包含TLS、TCP、IP三层；右侧为HTTP over QUIC，包含QUIC、UDP、IP三层。对比发现，HTTP over QUIC在TCP基础上，将TLS协议替换为QUIC协议，且在QUIC之上引入UDP协议。该图与上下文紧密相关，直观呈现了HTTP/3相较于HTTP/2在协议栈上的变化，为理解HTTP/3新特性中的零RTT建立连接等特性提供基础。](./imgs/ZKsGbk6fCoW1lTxDOwuc61Ucnee.png)
...
![图片展示了HTTP/3零RTT建立连接的流程。左侧为传统HTTP/2传输数据前的三次RTT过程，从TCP握手到HTTP数据传输需4RTT。右侧为HTTP/3仅需一次RTT传递数据，若缓存可将RTT减少至零。图中用不同颜色线条区分各阶段，如TCP握手、TLS握手、HTTP数据等，直观呈现了两者在连接建立和数据传输上的差异。](./imgs/Qu0wbIPlyoI9yBxo6TBcYszCnvc.png)
...
![图片展示了HTTP3握手流程中客户端与服务端的计算过程。客户端生成随机数b，计算B = g^b mod p，再用A、b、p生成秘钥K = A^b mod p；服务端接收g、p、A，计算A = g^a mod p，再用B、a、p得到秘钥K = B^a mod p。图片与上下文紧密相关，直观呈现了上下文中服务端和客户端计算秘钥K的步骤，帮助理解HTTP3握手中秘钥生成机制。](./imgs/DOsNbUWA7o9tFNxlC2Yc76o9nLg.png)
...
![图片展示了HTTP/2中连接ID（Connection ID）的应用。上方显示手机与服务器间存在两个红色叉号，表示中间连接丢失，连接被中断。下方则以手机与服务器间仅有一个蓝色框标识的“Connection ID”替代，表明HTTP/2通过Connection ID来标识连接，简化了连接管理，解决了HTTP/1.1及HTTP/2中因中间请求丢失导致的队头阻塞问题，体现了HTTP/2在连接管理上的优化。](./imgs/SYWgbLjCkoImXBx0PoJcdLS4neb.png)
...
![图片展示了HTTP/2中请求传输的示意图。左侧为服务器，中间有多个灰色方块代表网络设备，右侧是云朵，云朵内有红色方块，云朵右侧有三个蓝色方块，最右侧是手机。该图与上下文紧密相关，用于说明HTTP/2将请求请求粒度减小，通过Frame方式发送请求，但在TCP层Frame组合式组合得到Stream传输，一旦出现Stream中Frame丢失，其后方Stream都将被阻，直观呈现了这一过程。](./imgs/WCnkbOlhYoviIZxdoUHcuIuDnod.png)

- HTTP/2中将请求粒度减小，通过Frame的方式进行请求的发送。但在TCP层Frame组合得到Stream进行传输，一旦出现Stream中的Frame丢失，其后方的Stream都将会被阻塞。

![图片展示了HTTP/2中Stream传输过程中的Frame丢失问题。左侧为发送方，右侧为接收方，中间是传输管道。管道内有Stream1至Stream4，其中Stream2的Frame丢失。当应用层已读取Stream1，但未读取Stream2时，若Stream2的Frame丢失，应用层无法读取，且其后方的Stream3、Stream4也会被阻塞，直至应用层读取完毕。该图直观呈现了HTTP/2中Stream传输中因Frame丢失导致的阻塞问题。](./imgs/OBqbbX89YoZ2ZxxsUGick4t8n2b.png)
...
![图片展示了弱网络环境下HTTP/2传输速度比HTTP/1.1更慢的原因。图中“Record”包含多个“TCP Segment”，其中一个“TCP Segment”被红色框标注并有“丢失”标识。下方文字说明在弱网络下，丢包问题尤其严重，导致HTTP/2比HTTP/1.1更慢。该图与上文提到的HTTP/2默认采取TLS方式传输，网络环境较差时传输速度慢的内容相呼应，直观呈现了丢包问题对HTTP/2传输速度的影响。](./imgs/D04CbOkrdoWCbPxwdIpcf6m3nvd.png)
...
![图片展示了HTTP/3中重传请求的两种情况。左侧图中，客户端在RTO（重传超时）后发送重传请求，其Seq N与重传请求ACK N+1重合；右侧图中，客户端在RTO后发送重传请求，其Seq N与原始请求ACK N+1重合。该图与上下文介绍HTTP/3重传机制的内容相关，直观呈现了重传请求在不同情况下的Seq N和ACK N+1的对应关系。](./imgs/NZkRbXgxBoz2xrxPDG5cAXq8nOd.png)

- ACK Delay
- HTTP/3在计算RTT时健壮的考虑了服务端的ACK处理时延。

![图片展示了客户端与服务端的通信示意图，客户端发出请求，服务端响应，其间有“ACK Delay”标注。该图与上下文“HTTP/3在计算RTT时健壮的考虑了服务端的ACK处理时延”相关，直观呈现了客户端发送请求后，服务端响应并处理ACK时存在时延的情况，有助于理解HTTP/3对ACK处理时延的考虑。](./imgs/UjlEbVbvboqxOgxsYmIcVRq2nje.png)
