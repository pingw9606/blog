...
![图片展示了求解最优策略的数学推导过程。首先给出拒绝 r - 1 个应聘者的概率公式，接着通过求和与极限运算，得到 P(x)](./imgs/Ezl2b6blnoQEQFx1cmWckrQWnxT.png)
...
![图片展示了`pow(x,n)`函数的递归调用流程。当`n`不等于1时，函数调用自身`pow(x, n-1)`，并乘以`x`；当`n`等于1时，返回`x`。箭头和“recursive call until n==1”文字说明了递归调用直到`n==1`的过程，与上下文介绍的`pow(x,n)`递归调用自身直到`n==1`的内容相呼应，直观呈现了递归调用的逻辑。](./imgs/IR2TbBTeooAkE4xEoWEc2hTVnxc.png)
