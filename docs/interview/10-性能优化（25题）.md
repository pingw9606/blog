...
![图片展示了FCP（First Contentful Paint）时间与颜色编码、FPC分数的关系。FCP时间在0 - 2秒时，颜色编码为绿色（快），FPC分数为75 - 100；在2 - 4秒时，颜色编码为橙色（中等），FPC分数为50 - 74；超过4秒时，颜色编码为红色（慢），FPC分数为0 - 49。该图与上文介绍的FCP（最大内容绘制）指标相关，用于评估页面主要内容对用户可见时间，辅助进行性能检测。](./imgs/Vkt7bTMmUo2sk5xwJxKcIZdrnec.png)
...
![图片是一张表格，展示了LCP（最大内容绘制）时间与颜色编码的关系。LCP时间范围从0 - 2.5秒对应绿色（快），2.5 - 4秒对应橙色（中等），超过4秒对应红色（慢）。该表格与文档中介绍前端性能优化指标时提到的LCP指标相关，直观呈现了LCP时间与性能等级的对应关系，帮助理解LCP在性能评估中的标准。](./imgs/Amjibw0bPoqqjfxqW9zcolALnlh.png)
...
![图片展示了FID（首次输入延迟）时间与颜色编码的对应关系。当FID时间在0 - 100毫秒时，颜色编码为绿色（快）；在100 - 300毫秒时，颜色编码为橙色（中等）；超过300毫秒时，颜色编码为红色（慢）。该图片与文档中介绍FID首次输入延迟的内容相关，直观呈现了不同FID时间对应的性能等级，帮助理解FID性能评价标准。](./imgs/GmUXbXhE1o9JTKxVTarcMg0JnMh.png)
...
![图片是一张表格，展示了上下文介绍的TBT（Total Block Time）总阻塞时间的性能检测指标。表格中列出TTI时间（秒）与颜色编码对应关系：TTI时间在0 - 3.8秒时为绿色（快），3.9 - 7.3秒时为橙色（中等），超过 自动生成](./imgs/V2o1bUl1VopWBzxcylfcZ8M1nC7.png)
...
![图片是一张表格，展示了 addCriterion图片展示了TBT时间（毫秒）与颜色编码的关系。当TBT时间在0 - 300毫秒时，颜色编码为绿色（快）；在300](./imgs/GzD5b6MbkoDTHtxhbkacp8mtntf.png)
...
![图片展示了谷歌指定的`web`性能指标标准之一——视觉稳定性`CLS`的CLS时间（毫秒）与颜色编码对应关系。当CLS时间在0 - 0.1毫秒时，颜色编码为绿色（快）；在0.1 - 0.25毫秒时，颜色编码为橙色（中等）；超过0.25毫秒时，颜色编码为红色（慢）。该图与文档中介绍`Web Vitals`性能指标的内容相关，直观呈现了CLS时间的分类及对应颜色，帮助理解网站视觉稳定性性能的评价标准。](./imgs/ES8FbL7AgoJxjtxBdDGcnrGfn7Y.png)
...
![图片展示的是Chrome DevTools中Lighthouse选项卡界面。界面顶部有多个 addCriterionConsole、Lighthouse等多个选项卡。右侧有“Categories”分类，勾选了Performance、Progressive Web App、Best practices、Accessibility、SEO等类别。下方“Device”设备选项部分选中Mobile。底部提示“Identify and fix common problems that affect your site's performance, accessibility, and user”等。该图与文档中介绍前端性能优化指标及检测等内容相关，用于说明Lighthouse工具在性能检测中的应用。](./imgs/K9EMbYTOfol0S3xNqmPc1z3Snic.png)

可以看到淘宝的首屏时间是`0.6s`，可交互时间是`1.5s`，总阻塞时间是`10ms`。最大绘制时间是`1s`。通过这些指标就可以看到在哪方面存在性能瓶颈。

![图片展示的是网页性能测试结果页面。页面上方显示性能得分为97，下方列出多项性能指标，包括First Contentful Paint（0.6秒）、Time to Interactive（1.5秒）、Speed Index（1.2秒）、Total Blocking Time（10毫秒）、Largest Contentful Paint（1.0秒）和Cumulative Layout Shift（0.056）。页面右上角有切换显示模式的按钮。该图片与上文提到的前端性能优化指标及性能检测内容相关，直观呈现了淘宝网站的性能数据。](./imgs/EYqmbv7gDo0b4kx0HAuc4SMGnEd.png)

在下方会对渲染进行拍照截图，如果空白页面较多也能体现网站白屏时间过长。下面还会给一些优化建议。比如某些资源过大，加载时间过长等，当然这些建议不并一定都是对的，只是一些建议。

![图片展示的是WebPageTest的性能优化建议界面。上方有“View Original Trace”按钮。下方“Opportunities”部分建议消除渲染阻塞资源，可节省0.19秒。在“Diagnostics”部分，有3条诊断信息，分别是确保文本在webfont加载期间保持可见、不使用被动监听器以提高滚动性能、图像元素没有明确的width和height。这些内容与上下文提到的WebPageTest测试结果及优化建议相关，为网站性能优化提供参考。](./imgs/EYAkb8usYoe4E5xxG8ocSSbynGg.png)

最后是测试环境信息，不能制作一种环境的测试，要多环境测试。

![图片 addCriterion图片展示了WebPageTest的运行时设置界面。关键信息包括：URL为https://www.taobao.com/；Fetch Time为2021年6月16日14:26:00 GMT+8；Device为模拟桌面桌面；Network throttling设置为40 ms TCP RTT、10,240 Kbps吞吐量（模拟）；CPU throttling为1x慢速（模拟）；Channel为devtools；User agent ( (host)为Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) AppleWebKit/5377.36 (KHTML, like Gecko) Chrome/91.0.4472.101 Safari/537.36。该图片与文档中介绍WebPageTest性能测试的内容相关](./imgs/BRIybewPMolLRVxmQzWcPHcan0c.png)

2.WebPageTest

在线`web`性能测试工具(`https://www.webpagetest.org`), 提供多地点测试。他只能测试已经发布了的网站。输入需要测试的网页地址，点击`start test`按钮就开始测试了，可以选择测试地理位置，测试的浏览器等。

![图片展示的是WebPageTest网站界面，上方有“Test. Optimize. Repeat.”字样。界面中有“Advanced Testing”“Simple Testing”“Visual Comparison”“Traceroute”四个选项卡，当前选中“Advanced Testing”。下方有“Enter a website URL...”输入框，右侧有“Start Test”按钮。还有“Test Location”下拉框，显示“Virginia - EC2 (Chrome,Firefox)”，以及“Browser”下拉框，显示“Chrome”。该图片与文档中介绍前端性能优化指标及检测方法的内容相关，用于辅助说明性能检测工具的使用。](./imgs/RWcnbh6fXoph1Ux10m0chMOmnYc.png)

![图片展示的是网页性能测试平台的高级设置界面。界面中有“Test Location”和“Browser”选择框，分别可选择测试地点和浏览器。下方“Advanced Settings”区域有“Test Settings”选项卡被选中，显示了“Connection”（连接）、“Desktop Browser Dimensions”（桌面浏览器尺寸）、“Number of Tests to Run”（运行测试次数）、“Repeat View”（重复视图）和“Capture Video”（录制视频）等设置项，其中“Number of Tests to Run”可设置为1 - 9，当前为3，且“Capture Video”选项被勾选。该图片与文档中介绍网页性能测试平台设置的内容相关。](./imgs/OjyxbQ2aJo0CvqxyPy8cmkoHnCg.png)
...
![图片展示了Chrome浏览器任务管理器界面，列出了多个进程的内存占用空间、CPU、网络及进程ID等信息。如浏览器进程内存占用298MB，CPU占用2.6等；还有GPU进程、实用程序进程、标签页进程以及Adblock Plus等扩展程序进程的相关数据。该图与上文提到的通过Chrome浏览器任务管理器查看各进程关于GPU、网络和内存空间使用情况，以定位内存泄露或网络资源加载异常问题进程的内容相呼应。](./imgs/HF9lbcmt8oBjVUxvhJAcIYGPnOd.png)
...
![图片展示的是浏览器开发者工具中Network面板的界面。界面上方有多个标签，当前选中“Network”标签。下方表格列出了多个请求项，包括Name、Status、Protocol、Type、Size、Time、Content-Encoding、Waterfall等信息。其中，Name列显示了请求的资源名称，Status列显示状态码，Protocol列显示协议，Type列显示类型，Size列显示大小，Time列显示时间，Content-Encoding列显示编码，Waterfall列显示瀑布图。该图与文档中介绍Network面板获取网站资源请求情况的内容相关，直观呈现了相关数据。](./imgs/KjOrbuR0xoJ38ExEd0accgyhnIb.png)
...
![图片展示了Chrome DevTools中Coverage（覆盖率）界面。界面顶部有Console和Coverage标签，当前选中Coverage标签。下方有“Per function”下拉选项，以及刷新、下载、URL过滤等图标。界面底部显示“可以看到第一个文件有58%没有被使用到，第二个有95.2%没有使用到。”该图片与上文介绍的监控并统计网站应用运行过程中代码执行覆盖率情况的内容相关，直观呈现了覆盖率统计结果。](./imgs/IGx5bapY8otEOkxw2Woc2r58n3d.png)

![图片展示的是前端性能优化中Coverage监控统计结果。表格包含Type、Total Bytes、Unused Bytes、Usage Visualization四列。Type列显示为JS（per function）和CSS；Total Bytes列显示不同文件的字节大小；Unused Bytes列显示未使用的字节数及占比；Usage Visualization列以条形图形式呈现，红色部分代表已覆盖代码字节数，蓝色部分代表未覆盖代码字节数。该图与上下文介绍的Coverage监控统计内容相契合，直观呈现代码执行覆盖率情况。](./imgs/ANtjbDoayo5srqxTh1VceWGAnOd.png)
...
![图片展示的是Chrome浏览器中Memory面板的界面。界面上方有多个标签，当前选中“Memory”。下方有“Select profiling type”区域，可选择Heap snapshot、Allocation instrumentation on timeline、Allocation sampling三种内存分析类型。其中，Allocation instrumentation on timeline选项被选中。此外，还有“Select JavaScript VM instance”区域，显示了48.5 MB的内存占用情况。该图片与文档中介绍Memory面板用于分析内存占用情况，可快速生成堆内存快照的内容相关，图片直观呈现了Memory面板的操作界面。](./imgs/Ckg4bxFRCoCl2gxIk9IcL8BInAe.png)

结束后可以查看到内存占用大小，就可以对对应的模块进行优化。

![图片展示的是Chrome浏览器开发者工具中Memory面板的界面。界面顶部有多个标签，当前选中“Memory”标签。下方是内存占用情况的图表，显示了不同时间点的内存占用大小。图表下方是堆内存快照列表，列出了构造器、内部节点等各类对象，以及它们的构造器、距离、浅大小、](./imgs/EXGzbhVYnoQosfxS2KXc2yqznPe.png)

5.Performance

使用`Performance`面板主要对网站应用的运行时性能表现进行检测和分析，包括页面的每秒帧数，`CPU`的消耗和各种请求花费的时间。

![图片图片展示了Chrome浏览器开发者工具中Performance面板的界面。画面中“Performance”标签被红色框线突出显示，左侧有一个黑色圆点图标，图标旁有红色箭头指向，下方文字标注“开始记录”。该图片与](./imgs/LP7ibs8N2oP0iTxWTR7c9ntAnuf.png)

点击开始后等待两三秒就可以停止了。

![图片展示的是Chrome浏览器中`](./imgs/U77lbzreCo36jxxsIZzcYul1njd.png)

这里面就可以统计出网站的信息。

![图片展示的是Chrome DevTools中Performance面板的界面。界面上方有多个 addCriterion图片](./imgs/Agg0bGK2joeQBOx8o9YcQno5nlU.png)
...
![图片展示了浏览器性能监控界面，左侧有Frame RateRate、GPU raster、GPU memory等指标显示，如Frame Rate为24.6 fps，GPU memory使用67.8 MB，最大1073.7 MB。右侧文字提到为社区渐进升级付出的成本，官方透露升级到v18不会或很少改变现有代码，因CM是可选的。图片与上下文关系紧密，直观呈现了浏览器性能监控数据，辅助说明前端性能优化中性能检测相关内容。](./imgs/NHYQbh87Ho7OhWxamr1c5pTxnKf.png)

还可以使用性能监视器，这是一个事实的监视器。

`Ctrl + Shift + P` 输入 `monitor`

![图片 addCriterion图片展示了Chrome DevTools的Performance Monitor界面。上方显示总阻塞时间为1812.15ms（估计），并有“Learn more”链接。下方有多个选项卡，当前选中“Performance monitor”。界面中列出了CPU使用率、JS堆大小、DOM节点数等数据，如CPU使用率为2.8%，JS堆大小为42.1MB，DOM节点数为1,700等。该图与文档中介绍性能监视器的内容相关，直观呈现了性能监视器中可查看的性能指标数据。](./imgs/Smt3bLzlnoytyVxJf8vcbfC4nvY.png)
...
![图片展示了SPA首屏优化中静态资源本地缓存的两种请求方式。左侧“All in one request”表示一次性请求所有资源，右侧“Request per module”表示按模块分别请求资源。该图与文档中“静态资源本地缓存”解决方案部分相关，直观呈现了按模块分别请求资源的优化方式，有助于理解在SPA首屏优化中，通过按模块请求资源来减少请求次数，从而提升加载速度。](./imgs/HfoRbrJ7yohja4xdcKeceWunnZc.png)
...
![图片展示了前端首屏性能优化方案。左侧为资源加载优化，涵盖代码压缩、代码分割、Http缓存、Service Worker、CDN、DNS Prefetch、H2等方法；右侧为页面渲染优化，包括优化jsm代码、减少DOM数量、使用webworker、优化js、css代码、减少渲染、重绘、降低css选择器复杂度、使用requestAnimationFrame、优化动画过渡等。该图与上下文紧密相关，直观呈现了首屏优化的多种策略，供开发者根据项目情况选择使用。](./imgs/MyvYbGRktodnH1xelPKcUHBcnnc.png)
...
![图片展示了不同浏览器对Picture元素的使用情况。上方说明Picture元素是一种响应式图像方法，可基于分辨率、媒体查询和/或对特定图像格式的支持，控制用户代理呈现给用户的图像资源。下方表格按浏览器分类，显示各浏览器对Picture元素的使用百分比，如IE、Edge、Firefox、Chrome等，其中IE使用率最低为12%，Edge最高为87%，部分浏览器如Chrome、Firefox等使用率在80%以上。该图与文档中介绍Picture元素的内容相关，直观呈现其在浏览器中的应用情况。](./imgs/SG3ub4td3oJlTtxPHQocnpCsnCe.png)
