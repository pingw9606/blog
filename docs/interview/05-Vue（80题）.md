...
![图片展示的是Vue 3响应式原理示例中APP实例挂载后的页面效果。页面上显示“Price: 10”“Total: 20”“getTotal: 22”三行文本。其中，“Price”和“Total”是直接引用`price`和`total`属性的显示，而“getTotal”是通过`computed`属性计算得出的结果。该图片与上文通过创建APP实例和挂载APP实例展示页面中显示对应数值的内容相呼应，直观呈现了Vue 3响应式原理在实际页面中的应用效果。](./imgs/DntKbjWiiovLqixaxY6cvhDAnTd.png)
...
![图片展示了Vue 33中响应式设计原理中实现单个对象响应式变化的代码示例。左侧代码块中，定义了`price`、`quantity`和`total`变量，以及一个计算`total`的`effect`函数。右侧为`storage`存储区域，箭头指示将`effect`函数保存到其中。下方文字说明了代码执行流程：先保存代码，再运行此效果，最后触发运行所有保存的代码。该图与上下文紧密相关，直观呈现了实现单个对象响应式变化的代码逻辑及执行步骤。](./imgs/ORBeboYQVoyQTpxAaHdcdj9XnuP.png)
...
![图片展示了Vue 3中响应式设计原理中`depsMap`和`dep`的结构。左侧是`depsMap`，为Map类型，键为属性名（如quantity、price），值为`dep`对象；右侧是`dep`，为Set类型，包含副作用（如`total = product.price * product.quantity`）。箭头指向表明`depsMap`中属性对应的`dep`对象。该图与上下文紧密相关，直观呈现了上下文中提到的`depsMap`和`dep`的结构，帮助理解Vue 3响应式设计中如何存储依赖对象及副作用。](./imgs/DKn4bd9NAo0c5mxZpCVcDyuAnTe.png)
...
![图片展示了Vue 3中响应式设计原理中Proxy和Reflect的相关数据结构。左侧是`targetMap`，用于存储每个响应式对象的依赖；中间是`depsMap`，用于存储每个属性的依赖；右侧是`dep`，表示依赖，包含一组副作用，当值变化时应重新运行。图片与上文代码示例相呼应，直观呈现了代码中`targetMap`、`track()`、`trigger()`等概念在数据结构层面的对应关系，帮助理解响应式变化监听机制。](./imgs/XrG8b65iIoueqMximsdcSuWNnQb.png)
...
![图片展示了Vue 3 addCriterion中响应式设计原理的示例代码及流程。代码中定义了`product`对象，通过`Proxy`创建`proxiedProduct`代理对象，并设置`get`函数。当访问`proxiedProduct.quantity`时，`get`函数被调用，输出“Get was called with key = quantity”。箭头和文字标注了`product`、` addCriterion`proxiedProduct`、`get`函数及`quantity`等关键元素，直观呈现了响应式设计中通过`Proxy`拦截对象读取操作的过程。](./imgs/KpB8brTwfo31AKx1tjccCRU4nfe.png)
...
![图片展示了Vue 3中响应式设计原理中`reactive()`函数的执行流程。代码中定义了`product`对象，`effect`函数计算`total`，并打印`total`。当`product.quantity`变为3时，触发`trigger`函数。图片通过箭头和文字标注，说明了`track`函数收集依赖、`effect`函数执行、`trigger`函数触发等关键步骤，还展示了`targetMap`、`depsMap`和`dep`等数据结构，直观呈现了响应式机制的运行过程。](./imgs/JHRQbVIQ0o9NehxprUXc37Donvh.png)
...
![图片展示了Vue3中`packages/reactivity/src`目录下几个主要文件及其功能。其中`effect.ts`定义了`effect`/`track`/`trigger`；`baseHandlers.ts`定义了Proxy处理器（get和set）；`reactive.ts`定义`reactive`方法并创建ES addCriterionES6 Proxy；`ref.ts`定义reactive的ref使用的对象访问器；`computed.ts`定义计算属性的方法。图片与上文介绍的Vue3响应式设计原理](./imgs/S5HBb3GOSokZ0nx5pRMchpzKnze.png)
...
![图片展示了网页加载过程中的关键时间点，包括navigationStart、redirectStart、domainLookupStart等，以及DOMContentLoaded、load等事件。箭头连接各时间点和事件，直观呈现了从开始加载到页面渲染完成的流程。该图与文档中“关于计算首屏时间”部分内容相关，通过利用`performance.timing`提供的数据，可计算出首屏时间，此图帮助理解网页加载过程中的时间点和事件，辅助说明首屏时间的计算。](./imgs/WsJebBrbAoPyO1xVNqtcSS9wn5b.png)
...
![图片展示了两种资源请求方式对比。左侧是“All in one request”，呈现为一个由多个绿色方块横向相连的图形，代表所有资源在一个请求中获取；右侧是“Request per module”，有四个独立的绿色方块，每个方块下有箭头，代表每个模块单独请求。这与文档中SPA首屏优化相关内容呼应，说明在解决SPA首屏加载慢问题时，资源请求方式是可考虑的因素之一，不同请求方式对加载性能可能产生不同影响。](./imgs/YftybR81RoKtmBx8nY1czBYRnQd.png)
...
![图片展示了前端首屏性能优化方案，分为资源加载优化和页面渲染优化两部分。资源加载优化方面，包括代码压缩、图片压缩、代码拆分、HTTP缓存、Service Worker、CDN、HTTP前缀、DNS Prefetch、HTTP2、按需加载、增加缓存、优化资源加载时机、客户端内H5页可考虑高缓存包方式、优化资源、内容加载方式等。页面渲染优化方面，有优化HTML代码、css代码放在顶部、减少DOM量、使用Webworker、优化js、code代码、长任务异步执行、减少重复、重试、降低css选择器复杂度、使用requestAnimationFrame、优化动画效果等。](./imgs/BjYibkxynokL43xxh4AcAeehnFe.png)
...
![图片展示了前端工程化中部分构建工具的发展时间线。2012年有Grunt、Gulp/Browserify、Webpack；2015年新增Rollup；2018年有Parcel；2020年有Snowpack、Vite。图片位于介绍前端工程化发展迅速，各种构建工具层出不穷，目前Webpack仍占统治地位，且列出了部分构建工具的上下文之后，直观呈现了这些工具的发展时间，帮助理解前端构建工具的发展脉络。](./imgs/YFFLb9zOBoq2gMxtwiUcwuvanne.png)
...
![图片展示了Vite的Bundle based dev server原理。左侧从entry出发，经route分支，再由module聚合，最终形成Bundle，Bundle准备好后，Server ready。图中用箭头和颜色区分不同模块，如绿色代表route和module，紫色代表Bundle，直观呈现了Vite构建和加载模块的过程，与文档中介绍Vite原理的内容相呼应，帮助理解其按需加载机制。](./imgs/L6C1bjsIeoekKjxC1oAchUyZnqf.png)

`Vite`直接将源码交给浏览器，实现`dev server`秒开，浏览器显示页面需要相关模块时，再向`dev server`发起请求，服务器简单处理后，将该模块返回给浏览器，实现真正意义的按需加载。

![图片展示了Vite的Native ESM based dev server原理。左侧“Server ready”表示服务器就绪，中间“entry”为入口，接收HTTP请求。右侧以“route”和“module”为节点，呈现动态导入（code split point）的模块结构，箭头指示模块间关系。该图与文档中“实现原理”部分相关，直观呈现了Vite开发服务器的工作流程，帮助理解其基于Native ESM的开发服务器机制。](./imgs/ExdsbmYagoF6dOxBMTgcemdinpe.png)
...
<sheet sheet-id="hFsEG0" token="ThQZs0oivhyV23tLzsechKkPnhx"></sheet>
...
![图片为一个饼图，标题为“GitHub star”。图中展示了Webpack、Vite、Glup、Rollup四个工具的GitHub star占比情况。其中Webpack占比40%，Vite占比25%，Glup占比21%，Rollup占比14%。该图与文档中介绍Vite相关内容上下文对应，直观呈现了Vite在GitHub star数量上的表现，与文档中提到的Vite推出以来备受关注、社区活跃、GitHub star数量激增至37.4K相呼应。](./imgs/KshNb3clEoc8YMxtkKrcZnxvn8g.png)
...
![图片展示了在浏览器控制台中对数组进行操作的示例。先是将arr\[1\]赋值为2，接着触发了set操作，显示key: 1 value: 2，最终返回2。这与上下文内容相关，上下文提到通过索引改变数组操作能被Object.defineProperty监测到，而图片正是通过实际操作验证了这一点，直观呈现了通过索引改变数组时触发的set操作情况。](./imgs/RTi7bqPkWoxN4FxWq6mc83jXnOe.png)

**测试说明**

通过索引改变arr[1]，我们发现触发了set，也就是Object.defineProperty是可以检测到通过索引改变数组的操作的，那Vue2.0为什么没有实现呢？是尤大能力不行？这肯定毋庸置疑。那他为什么不实现呢？

![图片展示的是关于Vue2.0不能检查数组变化问题的讨论。hfan提问为何在Vue中Object.defineProperty能对对象属性监听，但对数组元素监听不了，yxx990803回应是因为性能问题，性能代价和用户体验收益不成正比。hfan追问性能问题具体是什么，yxx990803表示现代浏览器配置较好，可监测几万乃至几十万个属性，但若数组长度受限，可能无法使用此功能。该图片与上下文紧密相关，直观呈现了讨论内容。](./imgs/VvF5bqHugojfRyxd0BGcOPFHn6d.png)
...
![图片展示了Vue页面渲染流程的示意图。起始于`_init`，接着调用`$mount`挂载组件，之后依次经过`mountComponent`、`vm.update(vm.render())`等步骤；其中`vm.render`会生成VNode，再经`render`、`createElement`等操作，最后通过`patch`及`createElm`递归创建节点树并挂载到页面上。图中箭头指示数据和操作的流向。该图是对上文Vue页面渲染流程文字内容的可视化总结。](./imgs/SAI7bINRjopdumxX4n9c6Zjyn3c.png)
...
![图片展示的是Vue警告信息，内容为“\[Vue warn\]: The computed property 'fullName' is already defined in data.”，即“计算属性fullName已经在data中定义”。该警告出现在<Home>组件中，具体位置为src/views/Home.vue，还涉及<App>组件和<Root>。此图片与上文提到的“计算属性fullName不能在data中定义”相呼应，直观呈现了在data中定义计算属性fullName时会报的错误提示。](./imgs/YzEzbiyFSoBbTTxSX2Ec01upndb.png)
...
![图片展示了Vue中computed和watch区别相关代码的打印结果。新值newVal和旧值oldVal均为对象，包含firstName和lastName属性，且属性值均为“Brooklyn”和“Beckham”。两个对象的__ob__属性均为Observer，value属性指向同一对象，dep属性为Dep，vmCount为0。get和set方法均为reactiveGetter和reactiveSetter。该图片与上文提到的打印结果对应，直观呈现了新旧值对象的结构，帮助理解深度监听无法监听到对象内部属性变化的原因。](./imgs/QdwHbPGYsomWUPxLFEzc6W9an0e.png)
...
1. 与 computed 属性配合使用，computed 返回想要监听的属性值，watch 用来监听
...
1. **mapAcions：把actions里面的方法映射到methods中**
...
1.  **mapMutations：把mutations里面的方法映射到methods中**
...
1. **mapGetters:把getters属性映射到computed身上**
...
![图片展示了划分子模块并开启命名空间后的Vue store实例结构。在子模块中添加了`namespace : true`后，store实例中`actions`属性下的函数名称为`todo/handleActionsDel`等，`state`属性下有`todo`对象，其中`inputVal`属性值为“123”。该图片与上文介绍vuex辅助函数使用及命名空间相关，直观呈现了模块化后store实例的变化情况。](./imgs/APMlbNS9uoTxo7xYzBZct9UfnDc.png)
...
![图片描述图片中的关键信息。图片展示的是Vue 3项目打包后的体积情况，显示编译成功耗时18393毫秒。文件“dist\\js\\chunk-vendors.28d0d835.js”大小为89.59 KB，压缩后为32.11 KB；文件“dist\\js\\app.3f482fc1.js”大小为.LayoutStyle�� addCriterion](./imgs/Ul9obCuxaoH52TxrfdecKHkJnkb.png)
...
![图片展示了Vue 3项目打包后的文件信息。左侧为文件名，有`dist\\js\\chunk-vendors.28d0d835.js`和`dist\\js\\app.94092e3d.js`；中间是文件大小，分别为89.59 KiB和2.07 KiB；右侧是Gzipped大小，分别为32.11 KiB和1.04 KiB。图片与上下文关系紧密，用于说明在Vue 3项目中，通过`Tree shaking`特性，对项目进行打包后体积的变化情况，直观呈现打包文件的大小信息。](./imgs/TAfEbPHGRoQSYmxDZoHcyoVwnTh.png)
...
![图片展示了Vue 3项目打包后的体积信息。左侧列出了文件名，右侧分别显示文件大小和压缩后的大小。具体为：`dist\\js\\chunk-vendors.de2030ce.js`文件大小为78.91 KiB，压缩后为29.62 KiB；`dist\\js\\app.c06b53e.js`文件大小为1.92 KiB，压缩后为0.93 KiB。该图片与上下文关系紧密，用于直观呈现Vue 3项目打包后的体积情况，辅助说明打包体积变化等内容。](./imgs/PeywbbrtvoO0Olxbmx9clFvZnWk.png)
...
![图片展示了Vue 3项目打包后的体积信息。上方显示“Compiled successfully in 8011ms”，表明编译成功且耗时8011毫秒。下方列出两个文件：`dist\\js\\chunk-vendors.19e22567.js`大小为79.05 KiB，压缩后为29.64 KiB；`dist\\js\\app.00e83bfe.js`大小为2.15 KiB，压缩后为1.00 KiB。该图片与上下文关系紧密，直观呈现了在未引入`computed`和`watch`属性时，Vue 3项目打包后的体积情况，为后续对比体积变化提供参考。](./imgs/HHtNbNhluo4mprxL68Wc4oShnXb.png)
...
![代码/伪代码 @@@@ 图片展示了Options API相关代码。代码中定义了data、methods、computed、watch部分，每个部分下分别标注有功能A或功能B，体现了不同功能在代码结构中的分布情况。](./imgs/TQ2tbKOiNorbttxRypEcRXUonng.png)
...
![图片展示了Vue3 Composition API中组件逻辑组织方式。左侧为组件，右侧是功能相关的API。组件通过箭头指向多个API，如`function`分别对应橙色、绿色、蓝色、紫色的API。这与文档中介绍的Composition API“根据逻辑功能组织，一个功能所定义的所有API会放在一起，高内聚低耦合”的特点相契合，直观呈现了组件与功能API的关联关系。](./imgs/QBJAbZKDhoQx5KxREnYcdmS2n4c.png)
...
![图片展示了Vue2中使用mixin复用逻辑的代码示例。上方是`mixin.js`文件，包含`getWeather`和`getWeatherForecast`两个函数。下方是`Weather.vue`组件代码，通过`mixin`引入`getWeather`函数，使用`getWeatherForecast`函数获取天气预报数据。该图片与上下文紧密相关，通过对比说明在Vue2中使用mixin复用逻辑的情况，为后续介绍Composition API在逻辑组织和复用方面的优势做铺垫。](./imgs/DBgPbXToCopSLMxziENcdly6nVm.png)
...
![图片展示了Vue2.x的Options API与Vue3.0的Composition API在代码组织上的对比。左侧为Options API，代码块较多，颜色区分明显；右侧为Composition API，代码块较少，且数据来源清晰，命名冲突问题得到解决。该图与上下文紧密相关，直观呈现了Composition API在逻辑组织方面的优势，即修改一个功能时只需跳到控制该属性的方法中，避免了在文件中跳来跳去的问题。](./imgs/AjoTbofOHoNXwwx2Co2cEAF6nCd.png)
...
![图片展示了Vue组件渲染流程。上方有“Component Render Function”和“Watcher”两个关键部分，“Watcher””下有“Data”及“getter”“setter”等。下方是“Virtual DOM Tree”。箭头表示触发重渲染、收集依赖、通知等操作，还有一条虚线箭头从“Watcher”指向“Virtual DOM”，表明触发重渲染。该图与上文介绍的Vue组件渲染过程相关，直观呈现了组件渲染时依赖收集、触发重渲染等关键环节。](./imgs/FSv8b7kW0oPLW1xSqWVcOL07ndb.png)
...
![图片展示了Vue3中静态提升的原理。左侧代码为`<div><p>HelloWorld</p><p>{{msg}}</p></div>`。右侧是对应的虚拟DOM结构，`div`下有两个`p`标签，左侧`p`标签文本为`text:HelloWorld`，右侧`p`标签文本为`text:newMsg`且有`flag:1`标记。红色箭头指向右侧`p`标签，说明已标记静态节点的`p`标签在`diff`过程中不会比较，进一步提高性能，与上下文介绍的静态提升优化内容相呼应。](./imgs/FoN2bh5Wyocwj4xIORUcqwZanDy.png)
...
![图片展示了Vue3源码的目录结构。在packages目录下，有compiler-core、compiler-dom、compiler-sfc、compiler-ssr、reactivity、runtime-core、runtime-dom、runtime-test、server-renderer、shared、size-check、template-explorer、vue等子目录。其中，compiler-ssr、reactivity、runtime-core、runtime-dom、runtime-test、server-renderer、shared、size-check、template-explorer、vue等目录有箭头指向，表明这些目录可进一步展开查看。该图片与文档中介绍Vue3源码管理的内容相关，直观呈现了Vue3源码的模块拆分情况。](./imgs/IyDLbVRzno0lttx56jmc7Rytnsf.png)
...
![图片展示了`Options API`和`Composition API`在逻辑组织方面的对比。左侧`Options API`代码中，组件的逻辑分散在`setup`、`mounted`、`beforeDestroy`等生命周期钩子中，代码量较多且还存在命名冲突问题。右侧`Composition API`代码中，逻辑集中于`setup`函数内，代码量减少且命名清晰，如����������图片展示了`Options API`和`Composition API`在逻辑组织方面的对比。左侧`Options API`代码中，组件的逻辑分散在`setup`、`mounted`、`beforeDestroy`等生命周期钩子中，代码量较多且存在命名冲突问题。右侧`Composition API`代码中，逻辑集中于`setup`函数内，代码量减少且命名清晰，代码结构更加简洁明了。](./imgs/JUkDbzmQYoUymvxCpVzccpDPnrc.png)
...
![图片展示了跨域请求的两种情况。左侧为Web文档，包含图像和Canvas，图像来自domain - a . com，Canvas中的图像来自domain - b . com 。右侧是两个Web服务器，分别对应domain - a . com和domain - b . com 。箭头表示请求方向，主请求定义了来源，同源请求（总是允许）和跨源请求（由CORS控制）被明确标注。该图与上下文介绍的CORS（跨域资源共享）概念相关，直观呈现了CORS在请求控制中的作用。](./imgs/DJVvbNud9o7TGOxlxRUczBbznxc.png)
...
![图片展示了Vue中`diff`算法更新新旧`VNode`节点的示例。左侧为旧节点，右侧为新节点，节点用不同颜色圆圈标识。旧节点从左至右依次为D、E、F、G，新节点从左至右依次为D、E、F、G、C、B、A。通过`diff`算法，新旧节点比较时，发现旧节点D与新节点D相同，直接复用旧节点D作为`diff`后的第一个真实节点，同时旧节点`endIndex`移动到C，新节点的`startIndex`移动到了C。](./imgs/BEgbbTzoUoHHtBxvehRcizEinhe.png)

1. 比较的过程中，循环从两边向中间收拢

![图片展示了新旧`VNode`节点示例，用于说明`vue`通过`diff`算法更新的过程。上方“old”列出了A、B、C、D四个节点，下方“new”列出了D、C、E、A、B、F六个节点。左右两侧有蓝色箭头，分别指向新旧节点的起始和结束位置。该图与上下文紧密相关，直观呈现了新旧`VNode`节点的结构，为后续通过`diff`算法更新节点的示例提供可视化参考。](./imgs/MFqwb3dePoEr3OxctbEcPaZ7nXe.png)

下面举个`vue`通过`diff`算法更新的例子：

新旧`VNode`节点如下图所示：

![图片展示了新旧`VNode`节点示例，用于说明`vue`通过`diff`算法更新的过程。上方标注`old:`，下方标注`new:`，分别呈现旧节点和新节点的结构。旧节点从左至右依次为A、B、C、D，新节点从左至右依次为D、C、E、A、B、F。每个节点旁标注`startIndex`和`endIndex`。该图与上文提到的`vue`中`diff`算法应用于新旧`VNode`节点比较的内容相呼应，直观呈现节点结构，辅助理解`diff`算法在`vue`中的应用。](./imgs/DpLybVym1o0CDnxVJ5UcNntlnZb.png)

第一次循环后，发现旧节点D与新节点D相同，直接复用旧节点D作为`diff`后的第一个真实节点，同时旧节点`endIndex`移动到C，新节点的 `startIndex` 移动到了 C

![图片展示了Vue中`diff`算法第一次循环后的状态。上方标注`old`，有A、B、C、D四个节点，D为蓝色，其余为绿色；下方标注`new`，有D、C、E、A、B、F六个节点，D、C为蓝色，其余为绿色。下方标注`diff`，有D、A、B、C四个节点，D为蓝色，其余为绿色。图片中`startIndex`和`endIndex`分别标注在对应位置。该图与上文提到的`diff`](./imgs/GO0cbPVQNo1CJZxjnVAcqOrvnjf.png)

第二次循环后，同样是旧节点的末尾和新节点的开头(都是 C)相同，同理，`diff` 后创建了 C 的真实节点插入到第一次创建的 B 节点后面。同时旧节点的 `endIndex` 移动到了 B，新节点的 `startIndex` 移动到了 E

![图片展示了Vue中`diff`算法更新的第二次循环情况。上方标注“startIndex”和“endIndex”，呈现旧节点和新节点的结构。旧节点从A到D，新节点从D到F。中间“startIndex”处新节点C与旧节点C相同，直接复用旧节点C作为`diff`后的第一个真实节点，同时旧节点`endIndex`移动到B，新节点的`startIndex`移动到了E。下方“diff”处展示了更新后的节点结构，包括D、C、A、B。该图与上文介绍的`diff`算法更新例子相呼应，直观呈现了第二次循环的节点变化情况。](./imgs/HtBfbm5xqoieOOxxFalc7mopnPh.png)

第三次循环中，发现E没有找到，这时候只能直接创建新的真实节点 E，插入到第二次创建的 C 节点之后。同时新节点的 `startIndex` 移动到了 A。旧节点的 `startIndex` 和 `endIndex` 都保持不动

![图片展示了Vue中`diff`算法更新的第三次循环示例。上方标注`startIndex`和`endIndex`，呈现旧节点和新节点的结构。旧节点从A到D，新节点从D到F。新节点的`startIndex`为A，`endIndex`为F。`diff`后节点变为D、C、E、A、B，其中E为新节点，用橙色突出显示。此图与上文介绍的`diff`算法更新例子相呼应，直观呈现了第三次循环中节点的对应关系及变化情况。](./imgs/YGDab3L4zov2qcxI4IbcNYaanc9.png)

第四次循环中，发现了新旧节点的开头(都是 A)相同，于是 `diff` 后创建了 A 的真实节点，插入到前一次创建的 E 节点后面。同时旧节点的 `startIndex` 移动到了 B，新节点的` startIndex` 移动到了 B

![图片展示了Vue中diff算法的第四次循环情形。上方标注“startIndex”和“endIndex”，old节点为红色A，new节点为绿色D、C、E、A、B、F。diff部分，新节点A、B、F对应真实节点，旧节点A对应真实节点，新节点D、C、E对应真实节点。新节点的startIndex为F，endIndex为B。该图与上下文紧密相关，直观呈现了第四次循环中节点状态及diff操作，帮助理解数据变化时节点处理逻辑。](./imgs/Qs13bQYuVoLABHxjJdtcE3cwnJc.png)

第五次循环中，情形同第四次循环一样，因此 `diff` 后创建了 B 真实节点 插入到前一次创建的 A 节点后面。同时旧节点的 `startIndex `移动到了 C，新节点的 startIndex 移动到了 F

![图片展示了Vue中diff算法的第五次操作情况。。上方标注“old”“new”“diff”，分别对应旧节点、新节点、diff节点。旧节点为A、B、C、D，新节点为D、C、E、A、B、F，diff节点为D、C](./imgs/TzRkbnEK5ovVmvxuoq2co4Z1nGh.png)

新节点的 `startIndex` 已经大于 `endIndex` 了，需要创建 `newStartIdx` 和 `newEndIdx` 之间的所有节点，也就是节点F，直接创建 F 节点对应的真实节点放到 B 节点后面

![图片展示了Vue中diff算法的第六次操作。图中有六个节点，分别为D、C、E、A、B、F，其中A、B、C、D节点被红色、黄色、橙色、蓝色框选。文字说明因oldStartIndex > oldEndIndex，退出循环，补充new增加的节点F。该图与上下文紧密相关，是对上文“新节点的`startIndex`已经大于`endIndex`了，需要创建`newStartIdx`和`newEndIdx`之间的所有节点，也就是节点F，直接创建F节点对应的真实节点放到B节点后面”这一操作的直观呈现。](./imgs/SzRVbb33ColrsExS7ACcjP4anLf.png)
...
![图片展示了一段JavaScript代码，代码先通过`document.createElement('div')`创建一个`div`元素，然后将其属性拼接成字符串并打印日志。打印出的日志内容为该`div`元素的众多属性，如`align`、`title`等，还有大量事件属性如`onclick`、`onkeydown`等。这段代码及打印结果直观呈现了真实`DOM`节点属性的庞大与复杂，与上下文提到的“真实的`DOM`元素非常庞大，页面性能问题大多由`DOM`操作引起”相呼应，用以说明虚拟`DOM`产生的背景。](./imgs/RI0WbKbwloThh1xnO8fcXGofnvg.png)
...
![图片展示了一款8位游戏机，名为“FEIHAO”。其上有电源开关键、复位键等标识，其中电源开关键位于左侧，复位键在右侧。图片中 自动生成](./imgs/Q6habIOhCo57cbxt7Azcc5iVnbb.png)
...
![图片是一张关系图，展示了组件间通信的分类。图中有父子、兄弟、没有直接关系三种关系。父子关系用红色虚线圆圈标识，表示父组件与子组件间有数据传递；兄弟关系用红色实线箭头标识，表示同级组件间的数据传递；没有直接关系用红色虚线箭头标识，表示非关系组件间的数据传递。该图与上下文紧密相关，直观呈现了组件间通信的分类，帮助理解不同组件间数据传递的关系。](./imgs/Eeswbfy3voPSlpxKGflcTzhunYb.png)
...
![图片展示了Vue组件间通过props传递数据的原理。左侧为“父组件”，右侧为“子组件”，两者间有虚线箭头连接，标注“Prop自定义属性”。子组件内有“props显式声明”字样。该图与文档中“props传递数据”部分内容对应，直观呈现了父组件向子组件传递数据时，通过props自定义属性进行显式声明的场景，帮助理解props在组件间传递数据的机制。](./imgs/WU5ebRWZ7omgJIxd25rcUfNDn5e.png)
...
![图片展示了Vuex在Vue组件通信中的工作流程。Vuex作为一个容器，包含Actions、Mutations和State。Vue组件通过Dispatch触发Actions，Actions通过Commit调用Mutations，Mutations则通过Mutate操作State，State的变化会使Vue组件重新Render。此外，Vuex可与Backend API交互，并连接Devtools进行调试。该图与上下文紧密相关，直观呈现了上文所述Vuex在复杂关系组件数据传递中的作用及各部分的运作机制。](./imgs/Kx5gbpMxCoCQFExAkPfcCOjonme.png)
...
![图片展示的是Vue组件定义中data属性的警告信息。画面中以红色字体形式呈现警告，内容为“\[ \[/vue.js:634\[Vue warn\]: The "data" option should be a function that returns a per-instance value in component definitions.](./imgs/Bw8nbWwZGoexsIxVgA0cEXAHnnb.png)
...
![图片是一张表格，列出了Vue生命周期的11个阶段及其描述。包括beforeCreate、created、beforeMount、mounted、beforeUpdate、updated、beforeDestroy、destroyed、activated、deactivated、errorCaptured等。如beforeCreate在组件实例被创建之初，created完成数据观测等。该表与上下文紧密相关，是对上文提到的Vue生命周期8个阶段的补充说明，帮助理解各阶段的具体含义和作用。](./imgs/JVXbbmDXhoTDgPxQ45Dc8Shznrb.png)

**三、生命周期整体流程**

`Vue`生命周期流程图

![图片是Vue生命周期流程图，展示了从new Vue()到destroyed的整个生命周期过程。流程从new Vue()开始，依次经历beforeCreate、created、beforeMount、mounted、beforeUpdate、updated、beforeDestroy、destroyed等阶段，每个阶段有对应的生命周期钩子函数。图中还标注了部分关键点，如created阶段完成数据观测等，帮助理解Vue实例从创建到销毁的全过程。](./imgs/IONkbFbZuoMpSGxDYRUcUmrNnoc.png)
...
![图片是一张表格，列出了Vue生命周期的8个阶段及其描述。包括beforeCreate、created、beforeMount、mounted、beforeUpdate、updated、beforeDestroy、destroyed。如beforeCreate阶段执行时组件实例还未创建，常用于插件开发中执行初始化任务；mounted阶段初始化结束，dom已创建，可用于获取访问数据和dom元素等。该表与上文对Vue生命周期各阶段的介绍相呼应，是对各阶段更直观、全面的总结。](./imgs/QDXQblorDoDE3Gx7yWOcXy3vngd.png)
...
![图片展示了一个网页界面，上方显示“FBI: SECURE”及网址“http://www.fbi.org/”。界面中有“Name:”和“Email:”两个输入框，分别显示“Bob”和“bob@example.com”。该图位于介绍Vue双向绑定原理上下文之前，用以举例说明当用户填写表单时，视图层的状态被更新，若能自动更新数据层的状态，即为双向绑定，直观呈现了双向绑定在实际表单填写场景中的应用。](./imgs/I9WJbnjnPozrDSxgTTtcFtyTnFe.png)

当用户填写表单时，`View`的状态就被更新了，如果此时可以自动更新`Model`的状态，那就相当于我们把`Model`和`View`做了双向绑定关系图如下

![图片展示了双向绑定关系图。左侧JavaScript代码中，定义了包含name和email属性的数据对象data。右侧HTML部分有两行input标签，分别对应name和email属性。图中用红色箭头将JavaScript代码中的data与HTML中的input标签连接起来，直观呈现了双向绑定原理，即当数据发生变化时，视图会自动更新，反之亦然，体现了数据与视图的双向关联。](./imgs/AHt8bz1OWo7tOSxIwA5crPXqnTe.png)
...
![图片是Vue中双向绑定的流程图。new MVVM()初始化后，Observer劫持监听所有属性并通知变化，Compile解析指令、订阅数据变化并绑定更新函数，同时初始化视图。视图中对data中某key的使用形成依赖，Dep作为管家管理Watcher，Watcher添加到Dep成为订阅者。当数据变化时，Dep通知Watcher，Watcher调用更新函数，由Updater更新视图。此图直观呈现了Vue双向绑定的核心流程，与上文对双向绑定的原理描述相呼应。](./imgs/SxRbbmpi3oMtw5x0UokcAdlTnrb.png)
...
![图片展示了Vue编译流程图。从获取DOM开始，先遍历子元素，再编译节点，接着遍历属性。属性中，以k-开头的有k-text、k-html、k-model，分别对应处理textContent、处理innerHTML、监听input；以at开头的有at，对应绑定click。该图与上下文紧密相关，是对“对每个元素节点的指令进行扫描跟解析,根据指令模板替换数据,以及绑定相应的更新函数”这一编译步骤的可视化呈现，直观呈现了指令解析后的操作流程。](./imgs/FeTMbHdnAoYBulx9AkrcfYaxnyh.png)

![图片展示的是Vue中`Compile`类的构造函数和`compile`方法代码。构造函数接收`el`和`vm`参数，获取`el`对应的`dom`，若存在则调用`compile`方法。`compile`方法遍历`el`的子节点，判断节点类型，若为节点则输出其名称，若为插值文本则输出文本内容。若节点有子节点且长度大于0，则递归调用`compile`方法。该图片与上文介绍的Vue中双向绑定实现流程相关，是对`Compile`阶段中对每个元素节点的指令进行扫描跟解析等操作的代码呈现。](./imgs/RSQDbFDT9o2Lm4xG0NKc1EYTn1g.png)

**依赖收集**

视图中会用到`data`中某`key`，这称为依赖。同⼀个`key`可能出现多次，每次都需要收集出来用⼀个`Watcher`来维护它们，此过程称为依赖收集多个`Watcher`需要⼀个`Dep`来管理，需要更新时由`Dep`统⼀通知

![图片展示了Vue中依赖收集的过程。左侧蓝色区域是视图中用到`data`中某`key`的示例，如`<p>{{name1}}</p>`等。右侧是依赖收集的结果，包含三个`Watcher`（Watcher1、Watcher2、Watcher3），分别对应`name1`、`name2`、`name1`，以及两个`Dep`（Dep1、Dep2），其中`Dep1`的`deps`包含`Watcher1`和`Watcher3`，`Dep2`的`deps`包含`Watcher2`。此图直观呈现了依赖收集的原理，与上下文对依赖收集的解释相契合。](./imgs/WWzZbKnKPokJqRxkbBTcOjfOnTb.png)
...
![图片展示了`chars()`函数根据不同类型节点加上不同`type`，来标记`AST`节点类型的表格。表格包含两列，左侧为`type`，右侧为对应的`AST`节点类型。其中，`type`为1的对应节点类型是元素节点，`type`为2的对应节点类型是包含变量的动态文本节点，`type`为3的对应节点类型是没有变量的纯文本节点。该图片与上文提到的`chars()`函数根据不同类型节点加上不同`type`，来标记`AST`节点类型的内容相关，是对这一内容的直观呈现。](./imgs/Q5WVb2oucoa8vPxSbjUcJ9DQnbb.png)
...
![图片展示了Vue生命周期中关于模板编译的流程图。首先初始化注入和响应性，接着判断是否有el选项，箭 addCriterion图片展示了Vue生命周期中关于模板编译的流程图。首先初始化注入和响应性，接着判断是否有el选项，若有则进一步判断是否有template选项，若有则将template编译成render函数，若无则使用el挂载的模板编译el的outerHTML作为template。若无el选项，则在vm.$mount(el)被调用时，使用el挂载的模板编译el的outerHTML作为template。最后进入beforeMount阶段。该图与上文提到的渲染优先级相关，直观呈现了模板编译的逻辑流程。](./imgs/PoM9b1A6CoSw4uxChZIcxC5InZe.png)
...
![图片展示了Vue项目代码界面。左侧是文件目录，包含dist、src等文件夹及index.html等文件。右侧是代码编辑区域，显示App.vue文件内容，其中template部分包含div和p标签，style部分有scoped样式。该图片与上文搭建简单Vue项目测试CSS scoped原理的内容相关，直观呈现了代码结构，为后续测试现象做准备。](./imgs/D8UabW3zVokAYqxpXIzc8RUOn1g.png)

给个目录结构吧，代码并不是我们讲解的重点，如果需要源码测试的话后续我放到github上去。  
终端执行`npx webpack`输出dist目录，我们在浏览器打开index.html调试一下看看现象：

![图片展示了在浏览器开发者工具中调试Vue项目时的界面。左侧为HTML结构，显示了`<div>`、`<p>`等标签及其内容。右侧是“Elements”标签下的“Styles”选项卡，展示了选中标签的样式信息，如`p\[data-v-08310279\]`的样式设置。该图片与上文提到的在浏览器打开index.html调试Vue项目现象相关，直观呈现了调试时的样式查看情况，辅助说明了CSS scoped的原理及样式优先级等知识。](./imgs/FxtzbcQdko6o8Vx3eeDckYy4n9f.png)
...
![图片展示了`webpack.config.js`配置文件内容。文件中引入了`path`、`VueLoaderPlugin`和`HtmlWebpackPlugin`等模块，定义了`resolve`函数。`module.exports`部分设置`mode`为`development`，`devtool`为`source-map`，`entry`指向`./src/index.js`，`output`有`path`和`filename`等配置。`module`下的`rules`中，`test`为`/\\.\\vue$`的正则表达式，`loader`为`vue-loader`，还有两个未完全显示的规则。该图片与上文介绍`webpack.config.js`配置的内容相关，直观呈现了配置代码。](./imgs/HxcVbyq9coufiSx32q7c7PY2nxz.png)

**vue-loader工作流**

![图片展示了Vue-loader工作流。从`plugins`配置`new VueLoaderPlugin()`开始，通过`VueLoaderPlugin`修改`webpack`的`module.rules`配置，匹配.vue文件后，由`vue-loader`解析.vue文件，生成`template`、`js`、`style`、`custom`依赖请求，其中`style`依赖请求通过`@vue/component-compiler-utils`解析，生成`descriptor`对象。之后，`pitcher-loader`修改依赖请求，`xxx-loader`执行`loader.pitch`方法，整个流程环环相扣，呈现了Vue-loader在解析.vue文件时的处理逻辑。](./imgs/WMEqbv8btorP47xRlkAcNhh8nNd.png)

开启`node调试模式`进行查看阅读，package.json中配置如下：

![图片展示了`package.json`中`scripts`下的`debug`配置内容。配置为`"debug": "node --inspect-brk ./node_modules/webpack/bin/webpack.js"`，即在`node`调试模式下运行`webpack`。该图片与上文提到的在`webpack.config.js`中配置`node调试模式`进行查看阅读相呼应，是查看`webpack`配置时在`package.json`中找到的调试相关配置，用于后续对`vue-loader`操作.vue文件的解析结果进行调试阅读。](./imgs/Ttz0bJADNoTxPkxfA6GcVE4unKd.png)
...
![图片展示了在DevTools中对`node调试模式`进行查看阅读时的代码调试界面。左侧是代码编辑区域，显示了`index.js`文件内容，其中`staticRenderFns`部分被红色框突出显示。右侧是调试控制台，显示了`Vue`相关函数调用栈，如`Vue`、`Vue$patch`等。该图片与上文介绍`vue-loader`操作.vue文件时只关心`style`部分，逻辑在`lib/index.js`的内容相关，直观呈现了代码调试时的场景。](./imgs/CHsbb4sukoZISOxDwW7cbf29nxe.png)

可以看到解析带scoped属性的style的结果大概就是这样的：

![代码/伪代码 @@@@ 图片展示了编程代码的界面，左侧窗口显示多行代码，部分代码有蓝色高亮显示，右侧窗口呈现代码相关的结构、属性等信息。因未提供上下文，暂无法说明与标题及上下文关系。](./imgs/XPYGbYWdloCpyVxOlZccKIARnpf.png)
...
![图片展示了Vue中CSS scoped原理中`vue-loader`处理资源的代码片段。代码中，`selectBlock`函数接收`descriptor`、`loaderContext`、`query`等参数，判断`query.type`为`style`且`query.index`不为null时，从`descriptor.styles`获取对应样式内容，若`appendExtension`为true则更新`loaderContext.resourcePath`，最后调用`loaderContext.callback`返回样式内容及map信息。该代码与上下文介绍的vue-loader处理逻辑相关，展示了其处理样式资源的具体实现。](./imgs/KiEKb2donohAlCxh2fIcx2NVnSb.png)

> 可以看到vue-loader处理完后返回的就是style.content，也就是style标签下的内容，然后交给后续的loader继续处理

再来看一下`style-post-loader`是如何生成`data-v-hash:8`的

![图片展示的是`style-post-loader`处理样式代码的代码片段。首先通过`qs.parse`解析资源查询字符串，获取`query`。接着调用`compileStyle`函数处理样式，返回`code`、`map`、`errors`等信息。若存在错误，调用`this.callback`输出错误；若无错误，将`code`、`map`传递给`this.callback`。该代码与上下文介绍的`style-post-loader`生成`data-v-hash:8`的过程相关，是样式处理流程中的一部分。](./imgs/DzUCbq57bo4jFDxt32Bcrwdjnjd.png)

处理最终返回的code是这样的：

![图片展示的是`stylePostLoader.js`文件中处理`style`资源的代码。代码中首先通过`compileStyle`函数处理样式，获取`code`、`map`、`errors`等信息。若`errors`非空，调用`this.callback`处理错误；若`errors`为空，调用`this.callback`处理样式代码和映射信息。该图片与上文介绍的`style-post-loader`生成`data-v-hash:8`的处理流程相关，直观呈现了代码实现。](./imgs/Ksteb163toW0m9xj3gfcMB8hnzf.png)
...
- \_\_v_isVNode: *true*，内部属性，有该属性表示为Vnode
...
![图片展示了Vue组件中使用深度作用选择器的代码示例。在子组件中定义了.hello样式，父组件使用<style scoped>标签，通过.parent >>> .hello样式影响子组件样式。经过postCSS处理后的CSS代码中，.parent\[data-v-xxx\] .hello无法影响子组件，而.parent\[data-v-xxx\] .hello则可影响子组件。该图片与上下文紧密相关，直观呈现了深度作用选择器在Vue组件样式控制中的应用及效果。](./imgs/VjcZb09BfoJPPUxZ30BcDe6mngb.png)
...
![图片展示的是Vue相关代码片段，用于说明vue-loader做了哪些事情。代码中定义了injectStyles函数，对样式属性进行处理；还导入了normalizer模块，执行renderWithStyleInjection方法时先执行injectStyles方法，再执行原render方法。此代码与上文介绍的.vue文件中style标签有module属性时，向js代码注入injectStyle方法，以及css-loader处理样式内容转化为js代码等内容相呼应，展示了相关操作的具体实现。](./imgs/D4HIbslggoMTVxxDa0BcyuYFnAc.png)

1. 使用 css-loader 处理 .vue 文件 的 style 区域块，会将 style 区域块 中的样式内容， 转化为 js 代码， 如下:
...
1. 运行项目执行打包以后的js代码， 即 步骤1中的代码， 获取 render、staticRenderFns、scriptExprots、 style0、style1， 然后通过 normalizer 方法返回 组件完整配置项 - options。 在执行过程中，将 render 方法重新包装成 renderWithStyleInjection 方法。  
...
![图片展示了.vue文件经过vue-loader处理后生成的js代码。代码中从template区域块获取render、staticRenderFns方法，从script区域块获取组件的配置项对象，获取styles区域块的内容，以及获取styles（scoped）区域块的内容。还导入了normalizer模块，对组件配置项进行处理，返回组件配置项对象，包含data、props、render、staticRenderFns等。最后输出组件完整的配置项。该代码与上文提到的.vue文件处理样式丢失情况时生成的js代码相关，展示了具体代码内容。](./imgs/EzfsbU8Uxon6JpxvSpscWwdpnFY.png)
...
![图片展示了解决Vue项目中使用vue - loader处理.vue文件时样式丢失问题的一种配置方法。内容为在package.json文件中修改sideEffects属性，将其设置为\["*.vue"\]，以此告诉webpack.vue文件在使用tree shaking时会有副作用。这对应上文提到的第一种解决方法，有了此配置，webpack在处理.vue文件时不会使用tree shaking，从而避免样式丢失，但该方法存在script区域引入未使用模块时不会被删除的问题。](./imgs/NNnebmI3iotURcxUnFqcKC38nMd.png)
...
![图片展示了webpackConfig配置代码，针对.vue文件的style区域块和(style-lang-scss)区域块，分别设置了resourceQuery和sideEffects属性，明确其在使用tree shaking时会有副作用，使用不同loader处理。同时，package.json中sideEffects设置为false。该图片与上文介绍的通过rule.sideEffects指定具体模块在使用tree shaking时有副作用的内容相关，直观呈现了配置示例。](./imgs/N2MBbp7RVoZ5ENxBiQVcASYPnEe.png)
