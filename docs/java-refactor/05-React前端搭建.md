...
从 Vue 搬到 React 时，容易忍不住"顺手改改设计"。**别**。第一版目标是**像素级还原旧界面 + 功能对齐**——把旧站的全局样式（设计令牌、字体、配色、特色元素）、每个页面的结构、每个功能（连 Markdown 渲染、作者的分卷/章节管理、导入这些细节）都搬齐。原因：
- 老站是**已验收的需求**，照着搬不会漏功能、不会跑偏；
- 用户对旧界面有预期，擅自改设计等于给自己制造额外的验收和返工。

（这是踩过的坑：第一版图快做了个"能用但简化"的界面，结果功能不全、风格不对，只能推倒按旧版重做一遍。）优化留到复刻完、上线稳定之后再单独迭代。
